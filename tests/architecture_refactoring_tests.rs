...
use uveddi::analysis::{
    AnalysisEngine, AnalysisOrchestrator, AnalysisOrchestratorBuilder,
...
async fn test_architecture_complexity_reduction() {
    ...
    assert!(engine_lines < 500, "New engine has {} lines, should be < 500", engine_lines);
    ...
    let analysis_service_content = fs::read_to_string("src/analysis/services/analysis_service.rs").unwrap();
    let analysis_service_lines = analysis_service_content.lines().count();
    assert!(analysis_service_lines < 400, "AnalysisService should be < 400 lines, got {}", analysis_service_lines);
    
    let dependency_service_content = fs::read_to_string("src/analysis/services/dependency_service.rs").unwrap();
    let dependency_service_lines = dependency_service_content.lines().count();
    assert!(dependency_service_lines < 500, "DependencyService should be < 500 lines, got {}", dependency_service_lines);
    
    let performance_service_content = fs::read_to_string("src/analysis/services/performance_service.rs").unwrap();
    let performance_service_lines = performance_service_content.lines().count();
    assert!(performance_service_lines < 600, "PerformanceService should be < 600 lines, got {}", performance_service_lines);
    ...
    assert!(orchestrator_lines < 500, "Orchestrator should be < 500 lines, got {}", orchestrator_lines);
}
...
async fn test_performance_regression() {
    ...
    assert!(creation_time.as_millis() < 100, 
           "Engine creation took {}ms, should be < 100ms", creation_time.as_millis());
    ...
    assert!(batch_time.as_millis() < 1000,
           "Batch creation took {}ms, should be < 1000ms", batch_time.as_millis());
}
