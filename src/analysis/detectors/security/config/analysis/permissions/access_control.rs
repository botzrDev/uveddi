    ...
    fn build_access_control_rules() -> Result<Vec<AccessControlRule>, AnalysisError> {
        let rules = vec![
            AccessControlRule {
                ...
                pattern: Regex::new(r#"(?i)(allow|permit)[\s]*[:=][\s]*['"]?(all|\*|0\.0\.0\.0/0|::/0)['"]?"#)?,
                remediation: "Restrict access to specific users, groups, or IP ranges as needed.".to_string(),
                cwe_id: Some(284),
            ...
            AccessControlRule {
                ...
                severity: ConfigSeverity::High,
                pattern: Regex::new(r#"(?i)(auth|authentication)[\s]*[:=][\s]*['"]?(false|no|none|disabled|off)['"]?"#)?,
                remediation: "Enable proper authentication mechanisms for all services.".to_string(),
                cwe_id: Some(306),
            ...
            AccessControlRule {
                ...
                pattern: Regex::new(r#"(?i)(guest|anonymous|public)[\s]*[:=][\s]*['"]?(true|yes|enabled|on)['"]?"#)?,
            ...
            AccessControlRule {
                ...
                pattern: Regex::new(r#"(?i)(password|pwd|pass)[\s]*[:=][\s]*['"]?(admin|password|123456|default|guest|root)['"]?"#)?,
            ...
            AccessControlRule {
                ...
                severity: ConfigSeverity::High,
                pattern: Regex::new(r#"(?i)(api[_-]?key|token)[\s]*[:=][\s]*['"]?(public|open|unrestricted)['"]?"#)?,
            ...
            AccessControlRule {
                ...
                severity: ConfigSeverity::Medium,
                pattern: Regex::new(r#"(?i)(cors|cross[_-]?origin).*?(origin|allowed[_-]?origins)[\s]*[:=][\s]*['"]?\*['"]?"#)?,
            ...
            AccessControlRule {
                ...
                severity: ConfigSeverity::Medium,
                pattern: Regex::new(r#"(?i)(file[_-]?access|directory[_-]?listing)[\s]*[:=][\s]*['"]?(true|yes|enabled|unrestricted)['"]?"#)?,
            ...
            AccessControlRule {
                ...
                severity: ConfigSeverity::High,
                pattern: Regex::new(r#"(?i)(admin|management|console).*?(public|exposed|unrestricted)[\s]*[:=][\s]*['"]?(true|yes)['"]?"#)?,
                remediation: "Restrict administrative interfaces to authorized networks only.".to_string(),
                cwe_id: Some(284),
                owasp_category: Some("A01:2021 - Broken Access Control".to_string()),
            },
        ];
...
mod tests {
    ...
    fn test_default_credentials_detection() {
        ...
        let issues = checker.check_line(line, 1);
        assert!(issues.iter().any(|i| i.title.contains("Default Credentials")));
    }
