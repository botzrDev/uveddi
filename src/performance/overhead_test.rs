...
pub fn measure_statistical_overhead() {
    ...
    for _ in 0..10 {
        ...
        let _es = analyzer.effect_size(&small_data[..50], &small_data[50..]).unwrap();
    }
    let pipeline_avg = start.elapsed() / 10;
    println!("Complete analysis (100 points): {:?} per test", pipeline_avg);
    ...
    println!("\n--- Results ---");
    println!("Statistical analysis overhead: {:.1}x slower", overhead_ratio);
    println!("Overhead percentage: {:.1}%", overhead_percent);
    
    if overhead_percent < 500.0 { // 5x slower = 500% overhead
    ...
    println!("Total static overhead: {} bytes", analyzer_size + detector_size);
}
