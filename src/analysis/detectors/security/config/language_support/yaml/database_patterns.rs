        ...
        match value {
            YamlValue::Mapping(map) => {
                ...
                if is_db_config {
                    ...
                    if let Some(ssl_val) =
                        YamlParser::get_mapping_value(map, &["ssl", "ssl_enabled", "use_ssl"])
                    {
                    ...
                    if YamlParser::get_mapping_value(
                        map,
                        &["max_connections", "connection_limit", "pool_size"],
                    )
                    .is_none()
                    {
                        issues.push(
                    ...
                    if let Some(auth_method) = YamlParser::get_mapping_value(
                        map,
                        &["auth_method", "authentication", "auth"],
                    ) {
                    ...
                    if let Some(backup_config) =
                        YamlParser::get_mapping_value(map, &["backup", "backups"])
                    {
                        if let YamlValue::Mapping(backup_map) = backup_config {
                            if let Some(encryption) = YamlParser::get_mapping_value(
                                backup_map,
                                &["encryption", "encrypt"],
                            ) {
                    ...
                    if let Some(logging_config) =
                        YamlParser::get_mapping_value(map, &["logging", "log_config"])
                    {
                        if let YamlValue::Mapping(log_map) = logging_config {
                            if let Some(log_statements) = YamlParser::get_mapping_value(
                                log_map,
                                &["log_statements", "log_queries"],
                            ) {
