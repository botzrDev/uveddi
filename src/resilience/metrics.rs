...
impl MetricsCollector {
        ...
        let cleanup_handle = tokio::spawn({
            ...
            async move {
                loop {
                    ...
                    events.retain(|e| now.duration_since(e.timestamp).unwrap_or(Duration::ZERO) <= config.retention_duration);
    ...
    pub async fn get_metrics_snapshot(&self) -> ErrorMetrics {
        ...
        let (total_errors, errors_by_category, errors_by_severity) = {
            ...
            for event in events.iter() {
                *errors_by_category.entry(event.category.clone()).or_insert(0) += 1;
    ...
    async fn cleanup_old_events(&self) {
        ...
        events.retain(|e| now.duration_since(e.timestamp).unwrap_or(Duration::ZERO) <= self.config.retention_duration);
    ...
    fn format_as_influxdb(&self, metrics: &ErrorMetrics) -> String {
        let mut output = String::new();
        let timestamp = metrics.last_updated.duration_since(UNIX_EPOCH).unwrap().as_nanos();
...
mod tests {
    ...
    async fn test_error_recording() {
        ...
        assert_eq!(*metrics.errors_by_category.get("service_communication").unwrap(), 1);
    }
    ...
    async fn test_thread_safety() {
        ...
        let metrics = collector.get_metrics_snapshot().await;
        assert_eq!(metrics.total_errors, 1000, "All 1000 errors should be recorded across threads");
    }
    ...
    async fn test_memory_efficiency() {
        ...
        let metrics = collector.get_metrics_snapshot().await;
        assert_eq!(metrics.total_errors, 0, "All events should be cleaned up after retention period");
    }
