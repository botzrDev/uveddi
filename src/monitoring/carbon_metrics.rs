...
impl CarbonAwarenessCollector {
    ...
    pub fn start_tracking(
        ...
        info!(
            "Started energy tracking for {:?} workload: {}",
    ...
    pub fn stop_tracking(&self, workload_id: &str) -> Result<EnergyConsumptionMetrics> {
        ...
        self.workload_duration_histogram
    ...
    pub fn record_power_sample(
        ...
        if let Some(session) = sessions.get_mut(workload_id) {
            ...
            debug!(
                "Recorded power sample for {}: {:.2}W (CPU: {:.1}%)",
                workload_id, power_estimate, cpu_utilization
            );
        }
    ...
    pub fn generate_carbon_report(&self) -> CarbonFootprintReport {
        ...
        for metric in &metrics {
            let entry = workload_breakdown
                .entry(metric.workload_type.clone())
    ...
    fn generate_optimization_recommendations(
        &self,
        metrics: &[EnergyConsumptionMetrics],
    ) -> Vec<String> {
        ...
        let avg_energy: f64 =
            metrics.iter().map(|m| m.energy_wh).sum::<f64>() / metrics.len() as f64;
        let avg_co2: f64 =
            metrics.iter().map(|m| m.co2_emissions_g).sum::<f64>() / metrics.len() as f64;
        ...
        let ga_metrics: Vec<_> = metrics
            .iter()
            .filter(|m| m.workload_type == WorkloadType::GeneticAlgorithm)
            .collect();

        if !ga_metrics.is_empty() {
            let ga_avg_energy: f64 =
                ga_metrics.iter().map(|m| m.energy_wh).sum::<f64>() / ga_metrics.len() as f64;
...
mod tests {
    ...
    async fn test_energy_tracking_session() {
        ...
        collector
            .record_power_sample(&workload_id, 75.0, 1024.0)
            .unwrap();
        collector
            .record_power_sample(&workload_id, 80.0, 1100.0)
            .unwrap();
