...
impl AstProviderImpl {
    ...
    async fn parse_and_cache(&self, file_path: &Path) -> Result<Arc<tree_sitter::Tree>, UveddiError> {
        ...
        if let Some(tree) = parsed_file.tree {
            ...
            Err(UveddiError::AstError {
                ...
                suggestion: "Check if the file contains valid syntax for the detected language".to_string(),
                source: None,
            })
        }
    }
