...
impl SessionChecker {
    ...
    pub fn check_weak_session_config(&self, value: &YamlValue) -> Result<Vec<ConfigIssue>, AnalysisError> {
                    ...
                    "timeout" => {
                        if let Some(timeout) = session_val.as_i64() {
                            if timeout > 86400 { // More than 24 hours
                                ...
                                .with_tag("session-security")
                                .with_remediation("Consider shorter session timeouts for security"));
                            }
                        }
                    },
                    _ => {},
            ...
            if let Some(method) = method_val.as_str() {
                match method.to_lowercase().as_str() {
                    "basic" => {
                        ...
                        .with_remediation("Use stronger authentication methods like OAuth2 or JWT")
                        .with_cwe(308));
                    },
                    "plain" | "plaintext" => {
                        issues.push(ConfigIssue::new(
                ...
                if let Some(min_length) = policy.get(&YamlValue::String("min_length".to_string())) {
                    if let Some(length) = min_length.as_i64() {
                        if length < 8 {
                            ...
                            .with_remediation("Set minimum password length to at least 8 characters"));
                        }
                    }
                }
            }
...
"#;
        ...
        assert!(issues.iter().any(|i| i.title.contains("Insecure Session Cookie")));
        assert!(issues.iter().any(|i| i.title.contains("Session Cookie XSS Risk")));
        assert!(issues.iter().any(|i| i.title.contains("Session Cookie CSRF Risk")));
    }
...
"#;
        ...
        assert!(issues.iter().any(|i| i.title.contains("Basic Authentication")));
        assert!(issues.iter().any(|i| i.title.contains("Weak Password Policy")));
    }
}