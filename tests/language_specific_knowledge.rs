...
use uveddi::ai::knowledge::{
    ...
    schema::{AntiPatternCategory, SourceLanguage}, // PatternKnowledge removed - not used
                                                   // build_integration::KnowledgeLibraryBuilder, // Disabled for alpha
};
...
fn test_rust_knowledge_content() {
    let libraries = language_specific::create_language_specific_libraries();
    let rust_knowledge = libraries
        .get(&SourceLanguage::Rust)
        .expect("Rust knowledge should exist");

    // Should have patterns
    assert!(
        !rust_knowledge.patterns.is_empty(),
        "Rust should have patterns"
    );
    ...
    if let Some(god_object) = rust_knowledge.patterns.get("god_object") {
        ...
        println!(
            "✓ Rust god_object pattern has {} symptoms and {} solutions",
            god_object.symptoms.len(),
            god_object.solutions.len()
        );
    }
    ...
    if let Some(resource_leak) = rust_knowledge.patterns.get("resource_leak") {
        ...
        let solutions_text = resource_leak
            .solutions
            .iter()
    ...
    let idiom_names: Vec<_> = rust_knowledge.idioms.iter().map(|i| &i.name).collect();
    ...
    assert!(
        !rust_knowledge.frameworks.is_empty(),
        "Rust should have frameworks"
    );
    ...
    assert!(
        !rust_knowledge.stdlib_patterns.is_empty(),
        "Rust should have stdlib patterns"
    );

    println!(
        "✓ Rust knowledge has {} idioms, {} frameworks, {} stdlib patterns",
...
fn test_python_knowledge_content() {
    let libraries = language_specific::create_language_specific_libraries();
    let python_knowledge = libraries
        .get(&SourceLanguage::Python)
    ...
    if let Some(god_object) = python_knowledge.patterns.get("god_object") {
        ...
        let has_high_threshold = god_object.detection_methods.iter().any(|method| {
            if let uveddi::ai::knowledge::schema::DetectionMethod::MetricThreshold {
                threshold,
                ..
            } = method
            {
    ...
    let has_list_comp = python_knowledge
        .idioms
        .iter()
...
fn test_javascript_typescript_knowledge_content() {
    ...
    let has_async_idiom = js_knowledge
        .idioms
        .iter()
    ...
    let has_type_guards = ts_knowledge
        .idioms
        .iter()
...
fn test_java_knowledge_content() {
    ...
    if let Some(spring) = java_knowledge.frameworks.get("spring") {
        ...
        let practices_text = spring
            ...
            .join(" ");
        assert!(
            practices_text.contains("constructor injection")
    ...
    let has_builder = java_knowledge
        .idioms
        .iter()
    ...
    let has_optional = java_knowledge
        .stdlib_patterns
        .iter()
...
fn test_language_integration_system() {
    ...
    if let Some(god_object) = enhanced_patterns.get("god_object") {
        assert!(
            !god_object.language_contexts.is_empty(),
        ...
        println!(
            "✓ Enhanced pattern has completeness score: {:.2}",
...
fn test_language_detection_context() {
    ...
    if let Some(rust_context) = integrator.get_detection_context(SourceLanguage::Rust, "god_object")
    {
        assert_eq!(rust_context.language, SourceLanguage::Rust);
        assert!(!rust_context.specific_symptoms.is_empty());
        ...
        println!(
            "✓ Rust context has {} symptoms and {} tools",
    ...
    if let Some(python_context) =
        integrator.get_detection_context(SourceLanguage::Python, "god_object")
    {
        assert_eq!(python_context.language, SourceLanguage::Python);

        // Should have Python-specific tools
        let tools_text = python_context
            .tools
            .iter()
            .map(|t| t.as_str())
            .collect::<Vec<_>>()
...
fn test_language_indices_generation() {
    ...
    assert!(indices
        .language_to_patterns
        .contains_key(&SourceLanguage::Python));
    ...
    println!(
        "✓ Generated indices: {} languages, {} frameworks, {} tools",
    ...
    for pattern in &rust_patterns {
        assert!(
            pattern
                .language_contexts
                .contains_key(&SourceLanguage::Rust),
            "Pattern {} should have Rust context",
            pattern.universal.id
        );
    }
    ...
    for (language, knowledge) in &libraries {
        ...
        for (pattern_id, pattern) in &knowledge.patterns {
            assert!(
                ...
                "Pattern {} should have name",
                pattern_id
            );
            assert!(
                !pattern.symptoms.is_empty(),
                "Pattern {} should have symptoms",
                pattern_id
            );

            // Detection confidence should be reasonable
            assert!(
                pattern.detection_confidence >= 0.5,
                "Pattern {} should have reasonable confidence",
                pattern_id
            );
            assert!(
                pattern.detection_confidence <= 1.0,
                "Pattern {} confidence should not exceed 1.0",
                pattern_id
            );
        }
        ...
        println!(
            "  ✓ {} patterns, {} idioms, {} frameworks, {} stdlib patterns",
...
fn test_uv333_acceptance_criteria() {
    ...
    for lang in required_languages {
        ...
        assert!(
            !knowledge.frameworks.is_empty(),
            "{:?} should have framework knowledge",
            lang
        );
    }
...
fn test_language_specific_performance() {
    ...
    assert!(
        ...
        "Library creation should be fast, took {}ms",
    ...
    assert!(
        integration_time.as_millis() < 2000,
        "Integration should be fast, took {}ms",
        integration_time.as_millis()
    );
    ...
    assert!(
        enhancement_time.as_millis() < 5000,
        "Pattern enhancement should be fast, took {}ms",
        enhancement_time.as_millis()
    );
