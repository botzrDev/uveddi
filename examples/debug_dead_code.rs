...
"#;
    ...
    for symbol in &symbols {
        println!(
            "Symbol: {} (exported: {}, type: {:?}, confidence: {:.2})",
    ...
    for issue in &issues {
        ...
        println!(
            "  Contains 'used_function': {}",
            issue.description.contains("used_function")
        );
        println!(
            "  Contains 'unused_function': {}",
            issue.description.contains("unused_function")
        );
    ...
    println!(
        "Should NOT detect used_function: {}",
        !issues
            .iter()
            .any(|issue| issue.description.contains("used_function"))
    );
    println!(
        "Should detect _private_unused: {}",
        issues
            .iter()
            .any(|issue| issue.description.contains("_private_unused"))
    );
}
