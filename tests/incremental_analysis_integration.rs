...
async fn test_basic_incremental_analysis() {
    ...
    let (issues2, result) = engine.analyze_incremental(temp_dir.path(), config).await.unwrap();
    
    // Validate results
    assert_eq!(issues1.len(), issues2.len(), "Incremental analysis should produce same results");
    ...
    println!("Files reanalyzed: {} / {}", result.files_reanalyzed, result.total_files);
}
...
async fn test_change_detection_accuracy() {
    ...
    assert_eq!(initial_changeset.added.len(), 20, "Should detect all files as new");
    assert_eq!(initial_changeset.modified.len(), 0, "No files should be modified initially");
    assert_eq!(initial_changeset.deleted.len(), 0, "No files should be deleted initially");
    
    // Update file states
    detector.update_file_states_for_new_files(&initial_changeset.added).await.unwrap();
    ...
    assert_eq!(change_result.modified.len(), 3, "Should detect 3 modified files");
    assert_eq!(change_result.added.len(), 0, "No new files should be detected");
    assert_eq!(change_result.deleted.len(), 0, "No deleted files should be detected");
    
    // Check that the right files were detected as modified
    for modified_file in &modified_files {
        assert!(change_result.modified.contains(modified_file), 
            "Should detect {} as modified", modified_file.display());
    }
}
...
async fn test_dependency_impact_analysis() {
    ...
    assert!(graph.total_files() > 0, "Dependency graph should contain files");
    ...
    if main_file.exists() {
        ...
        assert!(!impact.directly_affected.is_empty(), "Should have directly affected files");
        assert!(impact.impact_score >= 0.0 && impact.impact_score <= 1.0, "Impact score should be between 0 and 1");
        assert!(!impact.processing_order.is_empty(), "Should provide processing order");
    }
}
...
async fn test_state_persistence() {
    ...
    let state = manager.create_new_state(project_root.clone(), analysis_config).await.unwrap();
...
async fn test_performance_improvement_target() {
    ...
    if result.was_incremental {
        ...
        let time_savings_percent = if full_time > incremental_time {
            ((full_time - incremental_time).as_millis() as f64 / full_time.as_millis() as f64) * 100.0
        } else {
            0.0
        };
        ...
        assert!(time_savings_percent >= 0.0, "Should not be slower than full analysis");
        
        // Validate that only a subset of files were reanalyzed
        assert!(result.files_reanalyzed < result.total_files, "Should reanalyze fewer files than total");
    } else {
...
async fn test_cache_invalidation() {
    ...
    // First run
    let (issues1, result1) = engine.analyze_incremental(temp_dir.path(), config.clone()).await.unwrap();
    
    // Modify files
    let modified_files = modify_random_files(&temp_dir, 3);
    
    // Second run
    let (issues2, result2) = engine.analyze_incremental(temp_dir.path(), config).await.unwrap();
    ...
    if result2.was_incremental {
        ...
        assert!(!change_summary.changed_files.is_empty(), "Should detect changed files");
        ...
        assert!(!intersection.is_empty(), "Should detect at least some of the modified files");
    }
