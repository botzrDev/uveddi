...
"#;
    ...
    mod stub_tests {
        ...
        #[test]
        fn test_large_class_detector_graceful_fallback() {
            ...
            assert!(
                result.is_ok(),
                "LargeClassDetector should not panic when tree-sitter is disabled"
            );
        }

        #[test]
        fn test_informative_skipping() {
            println!("SKIPPED: tree-sitter feature disabled - using fallback behavior for large class detection");
