...
impl SemanticAnalyzer {
    ...
    pub fn extract_features(
        ...
        debug!("Extracting semantic features from CFG with {} nodes", cfg.node_count());
                        ...
                        let (data_flow_patterns, more_features) = rayon::join(
                            ...
                            || {
                                let (api_usage_patterns, final_features) = rayon::join(
                                    ...
                                    || {
                                        let (structural_metrics, variable_usage_patterns) = rayon::join(
                                            || self.compute_structural_metrics(cfg, ast),
                                            || self.extract_variable_usage_patterns(ast, source),
                                        );
        ...
        let (cyclomatic_complexity, (data_flow_patterns, (api_usage_patterns, (structural_metrics, variable_usage_patterns)))) = other_features;
    ...
    fn compute_cyclomatic_complexity(&self, cfg: &ControlFlowGraph) -> u32 {
        ...
        edge_count.saturating_sub(node_count).saturating_add(2 * connected_components)
    }
    ...
    fn traverse_for_depth(&self, node: &Node, current_depth: &mut u32, max_depth: &mut u32) {
        ...
        let increases_depth = matches!(node_kind, 
            "if_expression" | "if_statement" | "for_statement" | "for_expression" |
            "while_statement" | "while_expression" | "loop_expression" | "block" |
    ...
    fn compute_feature_vector(
        ...
        for pattern in data_flow_patterns {
            *pattern_counts.entry(pattern.pattern_type.clone()).or_insert(0.0) += pattern.confidence;
        }
        
        features[8] = *pattern_counts.get(&DataFlowType::Assignment).unwrap_or(&0.0);
        features[9] = *pattern_counts.get(&DataFlowType::Transformation).unwrap_or(&0.0);
        features[10] = *pattern_counts.get(&DataFlowType::Aggregation).unwrap_or(&0.0);
        ...
        features[14] = api_usage_patterns.iter().map(|p| p.usage_count as f64).sum::<f64>();
...
impl WeisfeilerLehmanKernel {
    ...
    pub fn compute_similarity(
        ...
        debug!("Computing WL similarity between CFGs with {} and {} nodes", 
               cfg1.node_count(), cfg2.node_count());
    ...
    fn compute_histogram_similarity(&self, hist1: &HashMap<String, u32>, hist2: &HashMap<String, u32>) -> f64 {
...
impl HybridSimilarityScorer {
    ...
    pub fn compute_semantic_similarity(
        ...
        let cfg_score = self.wl_kernel.compute_similarity(cfg1.graph(), cfg2.graph());
    ...
    fn compute_structural_similarity(&self, features1: &SemanticFeatures, features2: &SemanticFeatures) -> f64 {
    ...
    fn compute_ast_pattern_similarity(&self, features1: &SemanticFeatures, features2: &SemanticFeatures) -> f64 {
        ...
        let api_similarity = self.compute_api_similarity(
            &features1.api_usage_patterns,
            &features2.api_usage_patterns,
        );
    ...
    fn compute_pattern_similarity(&self, patterns1: &[DataFlowPattern], patterns2: &[DataFlowPattern]) -> f64 {
...
mod tests {
    ...
    fn test_semantic_analyzer_creation() {
        ...
        assert!(analyzer.language_config.common_apis.contains(&"println!".to_string()));
    }
