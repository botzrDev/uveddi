...
impl PerformanceRegressionDetector {
    ...
    #[instrument(skip(self))]
    pub async fn record_metric(&self, metric_name: &str, value: f64, metadata: HashMap<String, String>) -> Result<Option<RegressionResult>> {
    ...
    #[instrument(skip(self))]
    async fn check_for_regression(&self, metric_name: &str, current_value: f64) -> Result<Option<RegressionResult>> {
        ...
        let analysis = self.analyze_regression(metric_name, baseline, current_value).await?;
    ...
    async fn update_baseline(&self, metric_name: &str, data_point: &MetricDataPoint) -> Result<()> {
        ...
        if baseline.historical_values.len() >= self.config.min_samples_for_baseline {
            let values: Vec<f64> = baseline.historical_values.iter().map(|dp| dp.value).collect();
    ...
    fn calculate_baseline(&self, metric_name: &str, data_points: &[MetricDataPoint]) -> Result<PerformanceBaseline> {
    ...
    async fn analyze_regression(&self, metric_name: &str, baseline: &PerformanceBaseline, current_value: f64) -> Result<RegressionAnalysis> {
    ...
    fn analyze_trend_direction(&self, historical_values: &VecDeque<MetricDataPoint>) -> TrendDirection {
    ...
    fn detect_change_points(&self, historical_values: &VecDeque<MetricDataPoint>) -> Result<Vec<ChangePoint>> {
        ...
        for i in window_size..(values.len() - window_size) {
            ...
            let combined_std = (calculate_std_dev(before_window, before_mean) + 
                              calculate_std_dev(after_window, after_mean)) / 2.0;
            
            if change_magnitude > combined_std * 2.0 { // 2 standard deviations
    ...
    fn generate_recommendations(&self, metric_name: &str, trend: &TrendDirection) -> Vec<String> {
        ...
        match trend {
            TrendDirection::Degrading => {
                recommendations.push("Consider rolling back recent changes if correlation exists".to_string());
                recommendations.push("Scale up resources if infrastructure limits are reached".to_string());
                recommendations.push("Optimize identified performance bottlenecks".to_string());
            }
            TrendDirection::Volatile => {
                recommendations.push("Investigate intermittent issues and error patterns".to_string());
                recommendations.push("Implement better load balancing or circuit breakers".to_string());
                recommendations.push("Add monitoring for resource spikes and anomalies".to_string());
            }
            _ => {
        ...
        if metric_name.contains("latency") || metric_name.contains("response_time") {
            recommendations.push("Profile critical code paths for optimization opportunities".to_string());
            recommendations.push("Consider caching strategies for frequently accessed data".to_string());
        } else if metric_name.contains("memory") {
            recommendations.push("Run memory profiling to identify leaks or excessive allocation".to_string());
            recommendations.push("Consider memory optimization techniques or garbage collection tuning".to_string());
        }
...
impl AlertManager {
    ...
    async fn send_regression_alert(&self, result: &RegressionResult) -> Result<()> {
        ...
        let should_alert = {
            ...
            if let Some(last_alert_time) = last_alerts.get(&result.metric_name) {
                let cooldown = self.config.severity_thresholds.get(&result.severity)
                    .copied()
                    .unwrap_or(Duration::from_secs(3600)); // Default 1 hour
                
                SystemTime::now().duration_since(*last_alert_time).unwrap_or_default() > cooldown
            } else {
                true
            }
        };
    ...
    async fn send_notification(&self, channel: &NotificationChannel, result: &RegressionResult) -> Result<()> {
        match channel {
    ...
    async fn send_slack_notification(&self, _webhook_url: &str, result: &RegressionResult) -> Result<()> {
    ...
    async fn send_email_notification(&self, _recipients: &[String], result: &RegressionResult) -> Result<()> {
    ...
    async fn send_webhook_notification(&self, _url: &str, _headers: &HashMap<String, String>, result: &RegressionResult) -> Result<()> {
...
mod tests {
    ...
    async fn test_baseline_calculation() {
        ...
        for i in 0..10 {
            ...
            detector.record_metric("test_metric", 100.0 + i as f64, metadata).await.unwrap();
        }
    ...
    fn test_regression_severity_determination() {
        assert_eq!(determine_regression_severity(5.0), RegressionSeverity::Minor);
        assert_eq!(determine_regression_severity(20.0), RegressionSeverity::Moderate);
        assert_eq!(determine_regression_severity(40.0), RegressionSeverity::Major);
        assert_eq!(determine_regression_severity(60.0), RegressionSeverity::Critical);
    }
}