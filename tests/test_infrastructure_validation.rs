...
async fn create_sample_file(dir: &TempDir, filename: &str, content: &str) -> Result<std::path::PathBuf, std::io::Error> {
...
async fn test_file_creation() {
    let temp_dir = temp_test_dir();
    let file_path = create_sample_file(&temp_dir, "test.rs", "fn main() {}").await.unwrap();
...
async fn test_multiple_files() {
    ...
    let rust_file = create_sample_file(&temp_dir, "test.rs", "fn main() {}").await.unwrap();
    let python_file = create_sample_file(&temp_dir, "test.py", "print('hello')").await.unwrap();
    let js_file = create_sample_file(&temp_dir, "test.js", "console.log('hello')").await.unwrap();
...
async fn test_subdirectory_creation() {
    ...
    let file_path = create_sample_file(&temp_dir, "src/main.rs", "fn main() {}").await.unwrap();
...
async fn test_rstest_integration(#[case] filename: &str, #[case] content: &str) {
    let temp_dir = temp_test_dir();
    let file_path = create_sample_file(&temp_dir, filename, content).await.unwrap();
...
async fn test_path_generation(#[case] relative_path: &str) {
    ...
    assert_eq!(full_path.file_name().unwrap().to_str().unwrap(), Path::new(relative_path).file_name().unwrap().to_str().unwrap());
}
...
async fn test_memory_cleanup() {
    ...
    let temp_paths = {
        ...
        // Create some files
        create_sample_file(&temp_dir, "test1.txt", "content1").await.unwrap();
        create_sample_file(&temp_dir, "test2.txt", "content2").await.unwrap();
