...
use crate::database::repositories::traits::{Repository, EventRepository};
...
impl Repository for SqliteEventRepository {
    ...
    async fn save(&self, _entity: &Self::Entity) -> Result<Self::Entity> {
        Err(UveddiError::database_error_msg("Event functionality not implemented"))
    }
...
impl EventRepository for SqliteEventRepository {
    async fn find_by_entity(&self, _entity_type: &str, _entity_id: i64) -> Result<Vec<LifecycleEvent>> {
        Ok(Vec::new())
    }

    async fn find_by_time_range(&self, _start: DateTime<Utc>, _end: DateTime<Utc>) -> Result<Vec<LifecycleEvent>> {
        Ok(Vec::new())
    }

    async fn create_event(&self, _entity_type: &str, _entity_id: i64, _event_type: &str, _description: Option<String>) -> Result<LifecycleEvent> {
        Err(UveddiError::database_error_msg("Event functionality not implemented"))
    }
}