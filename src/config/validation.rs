...
impl SmartConfigValidator {
    ...
    pub async fn validate(
    ...
    ) -> Result<ValidationResult, UveddiError> {
        ...
        if let Some(path) = project_path {
            self.validate_project_specific_settings(config, path, &mut warnings, &mut suggestions)
                .await;
        }
    ...
    fn validate_basic_structure(&self, config: &Config, errors: &mut Vec<ValidationError>) {
        ...
        if config.ollama_model.is_none()
            && config.dead_code.is_none()
            && config.large_classes.is_none()
        {
    ...
    fn validate_ai_config(
        ...
        suggestions: &mut Vec<ConfigSuggestion>,
    ) {
        if let Some(ref model) = config.ollama_model {
            ...
            if !model.contains(':') {
                errors.push(ValidationError {
                    ...
                    message: "Model name should include version tag (e.g., 'model:version')"
                        .to_string(),
                    severity: ErrorSeverity::Medium,
                    fix_suggestion: Some(format!(
                        "Change '{}' to '{}:latest' or specify a version",
                        model, model
                    )),
                });
            }
            ...
            if let Some(ref context) = self.context {
                ...
                if model != &optimal_model {
                    suggestions.push(ConfigSuggestion {
                        ...
                        description: format!(
                            "Based on your project size ({:?}), consider using a different model",
                            context.estimated_size
                        ),
                        before: Some(model.clone()),
                        after: optimal_model,
                        benefit: "Better performance and accuracy for your project size"
                            .to_string(),
            ...
            if deprecated_models
                .iter()
                .any(|&deprecated| model.starts_with(deprecated))
            {
            ...
            suggestions.push(ConfigSuggestion {
                ...
                description: "AI-powered analysis provides detailed explanations and suggestions"
                    .to_string(),
                before: None,
                after: "ollama_model = \"deepseek-coder:6.7b-instruct-q4_0\"".to_string(),
                benefit: "Get intelligent insights and explanations for detected issues"
                    .to_string(),
    ...
    fn validate_dead_code_config(
        ...
        if let Some(ref dc_config) = config.dead_code {
            ...
            if let Some(confidence) = dc_config.confidence_threshold {
                if confidence < 0.1 {
                    warnings.push(ValidationWarning {
                        ...
                        message: "Very low confidence threshold may produce false positives"
                            .to_string(),
                        impact: ImpactLevel::High,
                        recommendation:
                            "Consider using a threshold of at least 0.5 for reliable results"
                                .to_string(),
                    });
                } else if confidence > 0.95 {
                    warnings.push(ValidationWarning {
                        field: "dead_code.confidence_threshold".to_string(),
                        message: "Very high confidence threshold may miss actual dead code"
                            .to_string(),
                        impact: ImpactLevel::Medium,
                        recommendation:
                            "Consider using a threshold between 0.7-0.9 for balanced results"
                                .to_string(),
                    });
                }
            }
            ...
            if let Some(ref patterns) = dc_config.ignore_patterns {
                ...
                let missing_patterns: Vec<&str> = common_patterns
                    ...
                    .filter(|&&pattern| {
                        !patterns
                            .iter()
                            .any(|p| p.contains(&pattern[..pattern.len() - 3]))
                    })
                    .copied()
                    .collect();
                ...
                if !missing_patterns.is_empty() {
                    suggestions.push(ConfigSuggestion {
                        ...
                        description: "Exclude build directories and dependencies from analysis"
                            .to_string(),
                        before: None,
            ...
            suggestions.push(ConfigSuggestion {
                ...
                description: "Dead code detection helps identify unused functions and variables"
                    .to_string(),
                before: None,
    ...
    fn validate_large_classes_config(
        ...
        if let Some(ref lc_config) = config.large_classes {
            ...
            if let Some(max_loc) = lc_config.max_logical_loc {
                if max_loc < 50 {
                    ...
                    warnings.push(ValidationWarning {
                        ...
                        impact: ImpactLevel::Medium,
                        recommendation:
                            "Consider using 200-400 LOC as threshold for better detection"
                                .to_string(),
                    });
                }
            }
                    ...
                    if let Some(max_loc) = thresholds.max_logical_loc {
                        if max_loc == lc_config.max_logical_loc.unwrap_or(0) {
                            warnings.push(ValidationWarning {
                                ...
                                message: format!(
                                    "Language override for {} has same value as default",
                                    lang
                                ),
                                impact: ImpactLevel::Low,
                ...
                if let Some(current_loc) = lc_config.max_logical_loc {
                    if (current_loc as f64 - optimal_thresholds.max_logical_loc as f64).abs() > 50.0
                    {
                        suggestions.push(ConfigSuggestion {
                            ...
                            description: format!(
                                "Based on your project type ({:?}), adjust LOC threshold",
                                context.estimated_size
                            ),
                            before: Some(format!("max_logical_loc = {}", current_loc)),
                            after: format!(
                                "max_logical_loc = {}",
                                optimal_thresholds.max_logical_loc
                            ),
                            benefit:
                                "Better balance between detection accuracy and false positives"
                                    .to_string(),
                            priority: Priority::Medium,
                        });
                    }
            ...
            suggestions.push(ConfigSuggestion {
                ...
                description:
                    "Large class detection helps identify god objects and maintainability issues"
                        .to_string(),
                before: None,
                after: "[large_classes]\nmax_logical_loc = 250\nmax_methods = 25".to_string(),
                benefit: "Identify classes that violate single responsibility principle"
                    .to_string(),
                priority: Priority::High,
            });
        }
    ...
    fn generate_context_aware_suggestions(
        &self,
        config: &Config,
        ...
        for language in &context.languages {
            match language.as_str() {
                ...
                "javascript" | "typescript" => {
                    self.add_js_ts_specific_suggestions(config, suggestions)
                }
                _ => {}
            }
        }
        ...
        match context.team_size {
            ...
            TeamSize::Individual => {
                if config
                    .dead_code
                    .as_ref()
                    .map_or(true, |dc| dc.confidence_threshold.unwrap_or(0.8) < 0.9)
                {
                    suggestions.push(ConfigSuggestion {
                        category: SuggestionCategory::Performance,
                        title: "Higher Confidence for Solo Development".to_string(),
                        description: "Solo developers can use higher confidence thresholds"
                            .to_string(),
                        before: Some("confidence_threshold = 0.8".to_string()),
                        after: "confidence_threshold = 0.9".to_string(),
                        benefit: "Fewer false positives when you know your codebase well"
                            .to_string(),
        ...
        if context.has_ci {
            suggestions.push(ConfigSuggestion {
                ...
                description: "Optimize configuration for continuous integration environments"
                    .to_string(),
                before: None,
    ...
    fn add_rust_specific_suggestions(
        &self,
        _config: &Config,
        suggestions: &mut Vec<ConfigSuggestion>,
    ) {
    ...
    fn add_python_specific_suggestions(
        &self,
        _config: &Config,
        suggestions: &mut Vec<ConfigSuggestion>,
    ) {
    ...
    fn add_js_ts_specific_suggestions(
        &self,
        _config: &Config,
        suggestions: &mut Vec<ConfigSuggestion>,
    ) {
    ...
    async fn validate_project_specific_settings(
        &self,
        config: &Config,
        project_path: &Path,
                ...
                if let Some(ref patterns) = dc_config.ignore_patterns {
                    if !patterns.iter().any(|p| p.contains("node_modules")) {
                        suggestions.push(ConfigSuggestion {
                            ...
                            description:
                                "Detected package.json - add Node.js specific ignore patterns"
                                    .to_string(),
                            before: None,
                            after:
                                r#"ignore_patterns = ["node_modules/**", "dist/**", "build/**"]"#
                                    .to_string(),
                            benefit: "Significantly faster analysis by excluding dependencies"
                                .to_string(),
                            priority: Priority::High,
                        });
                    }
                ...
                if let Some(ref patterns) = dc_config.ignore_patterns {
                    if !patterns.iter().any(|p| p.contains("target")) {
                        suggestions.push(ConfigSuggestion {
                            ...
                            description: "Detected Cargo.toml - add Rust specific ignore patterns"
                                .to_string(),
                            before: None,
    ...
    fn suggest_optimal_thresholds(&self, context: &ProjectContext) -> OptimalThresholds {
        let base_thresholds = match context.estimated_size {
            ProjectSize::Small => OptimalThresholds {
                max_logical_loc: 200,
                max_methods: 20,
            },
            ProjectSize::Medium => OptimalThresholds {
                max_logical_loc: 250,
                max_methods: 25,
            },
            ProjectSize::Large => OptimalThresholds {
                max_logical_loc: 300,
                max_methods: 30,
            },
            ProjectSize::Huge => OptimalThresholds {
                max_logical_loc: 400,
                max_methods: 35,
            },
        };
    ...
    fn calculate_performance_score(
        ...
        let performance_suggestions = suggestions
            .iter()
    ...
    fn calculate_completeness_score(&self, config: &Config) -> f64 {
        ...
        if let Some(ref dc_config) = config.dead_code {
            ...
            if dc_config.confidence_threshold.is_some() {
                score += 5.0;
            }
            if dc_config.library_mode.is_some() {
                score += 5.0;
            }
            if dc_config
                .ignore_patterns
        ...
        if let Some(ref lc_config) = config.large_classes {
            ...
            if lc_config.max_logical_loc.is_some() {
                score += 5.0;
            }
            if lc_config.max_methods.is_some() {
                score += 5.0;
            }
            if lc_config.max_fields.is_some() {
                score += 2.5;
            }
            if lc_config.max_cyclomatic_complexity.is_some() {
                score += 2.5;
            }
            if lc_config
                .ignore_patterns
...
pub async fn validate_config_file(path: &Path) -> Result<ValidationResult, UveddiError> {
    ...
    validator
        .validate(&config, Some(path.parent().unwrap_or(path)))
        .await
}

/// Helper function to validate a configuration with project context
pub async fn validate_config_with_context(
    config: &Config,
    context: ProjectContext,
    project_path: Option<&Path>,
) -> Result<ValidationResult, UveddiError> {
