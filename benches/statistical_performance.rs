            ...
            group.bench_with_input(
                ...
                |b, data| {
                    b.iter(|| {
                        black_box(
                            detector
                                .detect_change_points_binary(black_box(data))
                                .unwrap(),
                        )
                    })
                },
...
fn bench_confidence_intervals(c: &mut Criterion) {
    ...
    for size in [50, 100, 500, 1000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("95_percent", size), &data, |b, data| {
            b.iter(|| black_box(analyzer.confidence_interval(black_box(data), 0.95).unwrap()))
        });
    }
        ...
        group.bench_with_input(
            ...
            |b, (baseline, current)| {
                b.iter(|| {
                    black_box(
                        analyzer
                            .effect_size(black_box(baseline), black_box(current))
                            .unwrap(),
                    )
                })
            },
        ...
        group.bench_with_input(BenchmarkId::new("full_analysis", size), &data, |b, data| {
            b.iter(|| {
                ...
                let confidence_interval =
                    analyzer.confidence_interval(black_box(data), 0.95).unwrap();
    ...
    group.bench_function("simple_threshold", |b| {
        b.iter(|| {
            for &value in &test_values {
                let is_regression =
                    (value - baseline_mean) / baseline_mean * 100.0 > simple_threshold;
