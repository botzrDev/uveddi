...
impl DatabaseAuditStore {
    ...
    async fn get_connection(&self) -> SecurityResult<rusqlite::Connection> {
        let conn = rusqlite::Connection::open(&self.db_path).map_err(|e| {
            SecurityError::DatabaseConnectionError {
                error: e.to_string(),
            }
        })?;
...
impl AuditStore for DatabaseAuditStore {
        ...
        let rows = stmt
            .query_map(&param_refs[..], |row| {
                ...
                Ok(AuditEvent {
                    ...
                    resource,
                    action,
                    outcome: outcome.parse().map_err(|_| {
                        rusqlite::Error::InvalidColumnType(
                            7,
                            "outcome".to_string(),
                            rusqlite::types::Type::Text,
    ...
    async fn count_events(&self, criteria: AuditQueryCriteria) -> SecurityResult<u64> {
        ...
        let count: u64 = conn
            .query_row(&query, &param_refs[..], |row| {
    ...
    async fn get_statistics(
        ...
        let rows = stmt
            .query_map([start_time.timestamp(), end_time.timestamp()], |row| {
...
impl AuditLogger {
    ...
    pub async fn verify_integrity(
        &self,
        event_ids: Vec<Uuid>,
    ) -> SecurityResult<IntegrityCheckResult> {
...
impl ChainHasher {
    ...
    pub fn update(&mut self, event: &AuditEvent) {
        ...
        self.hasher
            .update(event.timestamp.timestamp().to_string().as_bytes());
