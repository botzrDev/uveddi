...
async fn test_engine_construction_compatibility() {
    ...
    assert!(
        engine2.is_ok(),
        "AnalysisEngine::new_with_memory_cache() should work"
    );
    ...
    assert!(
        engine3.is_ok(),
        "AnalysisEngine::with_cache_path() should work"
    );
    ...
    assert!(
        engine4.is_ok() || engine4.is_err(),
        "Plugin construction should not panic"
    );
}
...
async fn test_detector_configuration_compatibility() {
    ...
    assert!(
        result.is_ok(),
        "Analysis with configured detectors should succeed"
    );
}

#[tokio::test]
async fn test_anti_pattern_types_compatibility() {
    let engine = AnalysisEngine::new_with_memory_cache().expect("Failed to create engine");
    ...
    for pattern_type in types {
        assert!(
            pattern_type.name.len() > 0,
            "Anti-pattern type should have a name"
        );
        assert!(
            pattern_type.description.len() > 0,
            "Anti-pattern type should have a description"
        );
        assert!(
            pattern_type.category.len() > 0,
            "Anti-pattern type should have a category"
        );
    }
}
...
async fn test_files_analyzed_tracking() {
    ...
    assert!(
        files_count <= 3,
        "Should not exceed the number of files created"
    );
}
...
async fn test_ast_cache_methods_compatibility() {
    ...
    assert!(
        ...
        "Cache metrics should still be a JSON object after clear"
    );
}
        ...
        Ok(mut engine) => {
            ...
            if has_support {
                ...
                assert!(
                    ...
                    "get_plugin_stats should not panic"
                );

                // Test resource monitoring
                let monitor_result = engine.monitor_plugin_resources().await;
                assert!(
                    monitor_result.is_ok() || monitor_result.is_err(),
                    "monitor_plugin_resources should not panic"
                );
            }
        }
        Err(_) => {
...
async fn test_analysis_results_structure() {
    ...
    for issue in &issues {
        ...
        assert!(
            issue.analysis_run_id >= 0,
            "analysis_run_id should be valid"
        );
        assert!(
            issue.anti_pattern_type_id >= 0,
            "anti_pattern_type_id should be valid"
        );
...
async fn test_multiple_engines_independence() {
    ...
    let engine1 = AnalysisEngine::new_with_memory_cache().expect("Failed to create first engine");
    let engine2 = AnalysisEngine::new_with_memory_cache().expect("Failed to create second engine");
    ...
    assert!(
        !types1.is_empty(),
        "First engine should return anti-pattern types"
    );
    assert!(
        !types2.is_empty(),
        "Second engine should return anti-pattern types"
    );
    assert_eq!(
        types1.len(),
        types2.len(),
        "Both engines should have same detector set"
    );
}
