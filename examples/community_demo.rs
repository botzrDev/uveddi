...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    db.log_activity(&alice.id, ActivityType::DashboardView, Some("Main dashboard accessed"), None, None)?;
    db.log_activity(&alice.id, ActivityType::AnalysisDownload, Some("Downloaded Rust project analysis"), None, None)?;
    db.log_activity(&alice.id, ActivityType::ReportGenerated, Some("Generated PDF report"), None, None)?;
    
    db.log_activity(&bob.id, ActivityType::Login, None, Some("10.0.0.50"), Some("Chrome/91.0"))?;
    db.log_activity(&bob.id, ActivityType::DashboardView, Some("Dashboard viewed"), None, None)?;
    db.log_activity(&bob.id, ActivityType::DocumentationView, Some("API documentation"), None, None)?;
    ...
    for member in &members {
        ...
        println!("   {} {} ({}) - {} - Last active: {:?}", 
                 ...
                 member.email,
                 member.role.as_str(),
                 member.last_active.map(|dt| dt.format("%Y-%m-%d %H:%M").to_string()).unwrap_or("Never".to_string())
        );
    }
