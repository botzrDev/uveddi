    ...
    fn build_privilege_escalation_rules() -> Result<Vec<PrivilegeEscalationRule>, AnalysisError> {
        let rules = vec![
            PrivilegeEscalationRule {
                ...
                remediation: "Require passwords for sudo access and limit sudo privileges.".to_string(),
                cwe_id: Some(250),
            ...
            PrivilegeEscalationRule {
                ...
                remediation: "Remove privileged mode and use specific capabilities instead.".to_string(),
                cwe_id: Some(250),
                owasp_category: Some("A01:2021 - Broken Access Control".to_string()),
            },
            ...
            PrivilegeEscalationRule {
                ...
                severity: ConfigSeverity::High,
                pattern: Regex::new(r#"(?i)(cap[_-]?add|capabilities)[\s]*[:=][\s]*['"]?ALL['"]?"#)?,
            ...
            PrivilegeEscalationRule {
                ...
                pattern: Regex::new(r#"(?i)(privileges?|grants?)[\s]*[:=][\s]*['"]?(all|admin|root|superuser)['"]?"#)?,
                remediation: "Grant only necessary database privileges to application users.".to_string(),
                cwe_id: Some(250),
                owasp_category: Some("A01:2021 - Broken Access Control".to_string()),
            },
            ...
            PrivilegeEscalationRule {
                ...
                severity: ConfigSeverity::Medium,
                pattern: Regex::new(r#"(?i)(suid|sgid|set[_-]?uid|set[_-]?gid)[\s]*[:=][\s]*['"]?(true|yes|1)['"]?"#)?,
            ...
            PrivilegeEscalationRule {
                ...
                severity: ConfigSeverity::High,
                pattern: Regex::new(r#"(?i)(cap[_-]?add|capabilities).*?(SYS_ADMIN|SYS_PTRACE|SYS_MODULE|DAC_OVERRIDE|NET_ADMIN)"#)?,
                remediation: "Remove dangerous capabilities like SYS_ADMIN and use alternatives.".to_string(),
                cwe_id: Some(250),
                owasp_category: Some("A01:2021 - Broken Access Control".to_string()),
            },
            ...
            PrivilegeEscalationRule {
                ...
                severity: ConfigSeverity::High,
                pattern: Regex::new(r#"(?i)(host[_-]?network|host[_-]?pid|host[_-]?ipc)[\s]*[:=][\s]*['"]?(true|yes)['"]?"#)?,
                remediation: "Use isolated namespaces instead of sharing host namespaces.".to_string(),
                cwe_id: Some(250),
                owasp_category: Some("A01:2021 - Broken Access Control".to_string()),
            },
            ...
            PrivilegeEscalationRule {
                ...
                severity: ConfigSeverity::Medium,
                pattern: Regex::new(r#"(?i)(allow[_-]?privilege[_-]?escalation)[\s]*[:=][\s]*['"]?(true|yes)['"]?"#)?,
...
mod tests {
    ...
    fn test_dangerous_capabilities_detection() {
        ...
        let issues = checker.check_line(line, 1);
        assert!(issues.iter().any(|i| i.title.contains("Dangerous System Capabilities")));
    }
