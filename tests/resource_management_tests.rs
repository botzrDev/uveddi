...
mod memory_tracker_tests {
    ...
    async fn test_memory_exhaustion() {
        ...
        match result {
            Err(ResourceError::MemoryExhausted {
                requested,
                available,
                limit,
            }) => {
...
mod analysis_orchestrator_tests {
    ...
    async fn test_concurrent_analysis_limiting() {
        ...
        let result = timeout(
            ...
            orchestrator.start_analysis("test_analysis_blocked", 100, AnalysisPriority::Normal),
        )
        .await;
    ...
    async fn test_analysis_priority_ordering() {
        ...
        let _execution1 = orchestrator
            .start_analysis("blocking_analysis_1", 100, AnalysisPriority::Normal)
            .await
            .unwrap();

        let _execution2 = orchestrator
            .start_analysis("blocking_analysis_2", 100, AnalysisPriority::Normal)
            .await
            .unwrap();
...
mod degradation_manager_tests {
    ...
    async fn test_manual_feature_override() {
        ...
        manager
            .set_feature_enabled(FeatureCategory::AiAnalysis, false)
            .await
            .unwrap();
        assert!(!manager.is_feature_enabled(&FeatureCategory::AiAnalysis));
        ...
        // Re-enable it
        manager
            .set_feature_enabled(FeatureCategory::AiAnalysis, true)
            .await
            .unwrap();
        assert!(manager.is_feature_enabled(&FeatureCategory::AiAnalysis));
    }
...
mod integration_tests {
    ...
    async fn test_degradation_under_pressure() {
        ...
        manager
            .force_degradation_level(DegradationLevel::Normal, "Recovery test".to_string())
            .await
            .unwrap();
