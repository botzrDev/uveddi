...
impl ObservabilityManager {
    ...
    pub async fn new(config: ObservabilityConfig) -> Result<Self> {
        ...
        let health_check_system = Arc::new(HealthCheckSystem::new(metrics.clone(), config.clone()));

        // Start health monitoring if enabled
        if config.health_check.enabled && config.health_check.background_monitoring {
            health_check_system
                .start_monitoring(config.health_check.check_interval)
                .await;
        }
    ...
    pub async fn record_analysis(
        ...
        if success {
            self.metrics.record_bugs_found(
                "various",
                "anti_pattern",
                language,
                issues_found as u64,
            ...
            self.metrics
                .record_error("analysis_failure", "high", "analysis_engine");
        }
        ...
        F: std::future::Future<Output = Result<T>>,
    {
        ...
        let result = self
            .analysis_tracer
            .trace_analysis_workflow(project_id, analysis_type, workflow_fn)
            .await;
    ...
    pub fn create_http_middleware(&self) -> HttpObservabilityMiddleware {
        HttpObservabilityMiddleware::new(self.metrics.clone(), self.tracing_manager.clone())
    }
...
impl HttpObservabilityMiddleware {
    ...
    pub async fn record_request(
        ...
        if status_code >= 400 {
            ...
            self.metrics
                .record_error(error_type, "medium", "http_server");
        }
