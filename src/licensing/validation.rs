...
pub fn validate_license_key(key: &str) -> Result<(), LicenseError> {
    ...
    for (i, part) in parts.iter().enumerate().skip(1) {
        if part.len() != 4 {
            return Err(LicenseError::invalid_key_format(format!(
                "Segment {} must be exactly 4 characters",
                i + 1
            )));
        }
        if !part.chars().all(|c| c.is_ascii_alphanumeric()) {
            return Err(LicenseError::invalid_key_format(format!(
                "Segment {} must contain only letters and numbers",
                i + 1
            )));
        }
    }
...
pub fn is_feature_allowed(feature: &str, tier: &LicenseTier) -> bool {
    match feature {
        ...
        features::LANG_LUA | features::LANG_SQL => *tier >= LicenseTier::Enterprise,
...
pub fn get_required_tier_for_feature(feature: &str) -> &'static str {
    match feature {
        ...
        features::LANG_LUA | features::LANG_SQL => "Enterprise",
