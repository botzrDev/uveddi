...
pub enum RenderingServiceError {
    ...
    #[error("Network bandwidth limit exceeded: {current_mbps}Mbps current, {limit_mbps}Mbps limit")]
    ...
    DiagramComplexityExceeded { reason: String, limit: u64, actual: u64 },
    
    /// Input size exceeds maximum allowed
    #[error("Input size limit exceeded: {size_bytes} bytes (limit: {limit_bytes} bytes)")]
    InputTooLarge { size_bytes: usize, limit_bytes: usize },
    ...
    ImageConversionError { from_format: String, to_format: String, reason: String },
    
    /// Rendering queue is full
    #[error("Rendering queue full: {queue_size} items (limit: {max_queue_size})")]
    QueueFull { queue_size: u32, max_queue_size: u32 },
}
...
impl RenderingServiceError {
    ...
    pub fn category(&self) -> ErrorCategory {
        match self {
            Self::ServiceUnavailable | Self::ConnectionTimeout { .. } | Self::RequestTimeout { .. } 
            | Self::NetworkError { .. } | Self::AuthenticationFailure { .. } | Self::RateLimitExceeded { .. }
            | Self::InvalidResponse { .. } => ErrorCategory::ServiceCommunication,
            
            Self::MemoryExhaustion { .. } | Self::CpuTimeout { .. } | Self::DiskSpaceExhaustion { .. }
            | Self::NetworkBandwidthLimit { .. } | Self::QueueFull { .. } => ErrorCategory::ResourceExhaustion,
            
            Self::InvalidMermaidSyntax { .. } | Self::DiagramComplexityExceeded { .. }
            | Self::InputTooLarge { .. } | Self::UnsupportedDiagramType { .. } => ErrorCategory::DataValidation,
            
            Self::SecurityValidationFailed { .. } | Self::MaliciousContentDetected { .. } => ErrorCategory::Security,
            
            Self::PuppeteerError { .. } | Self::ImageConversionError { .. } => ErrorCategory::ServiceSpecific,
        }
    }
    ...
    pub fn user_message(&self) -> String {
        match self {
            Self::ServiceUnavailable => "The diagram rendering service is temporarily unavailable. Please try again later.".to_string(),
            Self::ConnectionTimeout { .. } | Self::RequestTimeout { .. } => "The rendering request timed out. Please try again.".to_string(),
            Self::NetworkError { .. } => "A network error occurred. Please check your connection and try again.".to_string(),
            Self::AuthenticationFailure { .. } => "Authentication failed. Please check your credentials.".to_string(),
            Self::RateLimitExceeded { retry_after_seconds } => {
                match retry_after_seconds {
                    Some(seconds) => format!("Rate limit exceeded. Please wait {} seconds before trying again.", seconds),
                    None => "Rate limit exceeded. Please wait before trying again.".to_string(),
                }
            },
            Self::InvalidMermaidSyntax { line, details } => {
                match line {
                    Some(line_num) => format!("Invalid diagram syntax at line {}: {}", line_num, details),
                    None => format!("Invalid diagram syntax: {}", details),
                }
            },
            Self::DiagramComplexityExceeded { reason, .. } => format!("Diagram is too complex: {}", reason),
            Self::InputTooLarge { size_bytes, limit_bytes } => {
                format!("Input size ({} bytes) exceeds the maximum allowed ({} bytes)", size_bytes, limit_bytes)
            },
            Self::UnsupportedDiagramType { diagram_type } => format!("Unsupported diagram type: {}", diagram_type),
            Self::SecurityValidationFailed { .. } => "Security validation failed. Please review your diagram content.".to_string(),
            Self::MaliciousContentDetected { .. } => "Potentially malicious content detected. Please review your diagram.".to_string(),
...
impl RenderingServiceError {
    ...
    pub fn connection_timeout(timeout: Duration) -> Self {
        Self::ConnectionTimeout { timeout: timeout.as_millis() as u64 }
    }

    /// Creates a request timeout error
    pub fn request_timeout(timeout: Duration) -> Self {
        Self::RequestTimeout { timeout: timeout.as_millis() as u64 }
    }
...
mod tests {
    ...
    fn test_error_retryability() {
        ...
        assert!(!RenderingServiceError::AuthenticationFailure { reason: "invalid token".to_string() }.is_retryable());
        assert!(!RenderingServiceError::invalid_mermaid("syntax error", Some(5)).is_retryable());
        assert!(!RenderingServiceError::SecurityValidationFailed { reason: "malicious".to_string() }.is_retryable());
    }
    ...
    fn test_circuit_breaker_triggers() {
        ...
        assert!(RenderingServiceError::connection_timeout(Duration::from_secs(30)).should_open_circuit());
        assert!(RenderingServiceError::memory_exhaustion(1024, 512).should_open_circuit());
        
        // Should not open circuit
        assert!(!RenderingServiceError::AuthenticationFailure { reason: "invalid".to_string() }.should_open_circuit());
        assert!(!RenderingServiceError::invalid_mermaid("syntax error", None).should_open_circuit());
        assert!(!RenderingServiceError::RateLimitExceeded { retry_after_seconds: Some(60) }.should_open_circuit());
    }
    ...
    fn test_fallback_triggers() {
        ...
        assert!(RenderingServiceError::connection_timeout(Duration::from_secs(30)).should_trigger_fallback());
        assert!(RenderingServiceError::memory_exhaustion(1024, 512).should_trigger_fallback());
        
        // Should not trigger fallback
        assert!(!RenderingServiceError::invalid_mermaid("syntax error", None).should_trigger_fallback());
        assert!(!RenderingServiceError::SecurityValidationFailed { reason: "malicious".to_string() }.should_trigger_fallback());
    }

    #[test]
    fn test_error_severity() {
        assert_eq!(RenderingServiceError::ServiceUnavailable.severity(), ErrorSeverity::Critical);
        assert_eq!(RenderingServiceError::connection_timeout(Duration::from_secs(30)).severity(), ErrorSeverity::High);
        assert_eq!(RenderingServiceError::invalid_mermaid("syntax error", None).severity(), ErrorSeverity::Low);
        assert_eq!(RenderingServiceError::SecurityValidationFailed { reason: "test".to_string() }.severity(), ErrorSeverity::Security);
    }

    #[test]
    fn test_error_categories() {
        assert_eq!(RenderingServiceError::ServiceUnavailable.category(), ErrorCategory::ServiceCommunication);
        assert_eq!(RenderingServiceError::memory_exhaustion(1024, 512).category(), ErrorCategory::ResourceExhaustion);
        assert_eq!(RenderingServiceError::invalid_mermaid("test", None).category(), ErrorCategory::DataValidation);
        assert_eq!(RenderingServiceError::SecurityValidationFailed { reason: "test".to_string() }.category(), ErrorCategory::Security);
    }
