...
use uveddi::ai::knowledge::population::{populate_knowledge_library, populate_from_detector_analysis};
...
fn test_knowledge_library_creation() {
    ...
    assert!(library.universal_patterns.len() > 0, "Library should contain universal patterns");
    assert!(library.detection_context.len() > 0, "Library should have detection context mappings");
    assert!(library.solution_patterns.len() > 0, "Library should have solution pattern mappings");
    
    // Metadata validation
    assert!(!library.metadata.content_version.is_empty(), "Content version should be set");
    assert!(library.metadata.pattern_count > 0, "Pattern count should be positive");
    assert!(library.metadata.supported_languages.contains(&SourceLanguage::Universal), 
            "Should support universal patterns");
    
    println!("✓ Successfully created knowledge library with {} patterns", 
...
fn test_core_patterns_present() {
    ...
    for pattern_id in &expected_patterns {
        assert!(library.universal_patterns.contains_key(*pattern_id),
                "Core pattern '{}' should be present", pattern_id);
        
        let pattern = &library.universal_patterns[*pattern_id];
        assert!(!pattern.name.is_empty(), "Pattern '{}' should have a name", pattern_id);
        assert!(!pattern.definition.is_empty(), "Pattern '{}' should have a definition", pattern_id);
...
fn test_god_object_pattern_completeness() {
    let library = populate_knowledge_library().expect("Failed to populate knowledge library");
    let god_object = library.universal_patterns.get("god_object")
    ...
    // Content validation
    assert!(god_object.definition.len() > 100, "Definition should be comprehensive");
    assert!(god_object.symptoms.len() >= 5, "Should have multiple symptoms");
    assert!(god_object.solutions.len() >= 1, "Should have at least one solution");
    assert!(god_object.detection_methods.len() >= 1, "Should have detection methods");
    
    // Solution validation
    for solution in &god_object.solutions {
        assert!(!solution.title.is_empty(), "Solution should have a title");
        assert!(!solution.implementation.is_empty(), "Solution should have implementation details");
        assert!(solution.examples.len() > 0, "Solution should have examples");
    }
    
    // Tags validation
    assert!(god_object.tags.contains(&"object-oriented".to_string()), 
            "Should have object-oriented tag");
    assert!(god_object.tags.contains(&"design".to_string()), 
            "Should have design tag");
    
    // Related patterns
    assert!(god_object.related_patterns.contains(&"large_class".to_string()),
...
fn test_tight_coupling_pattern_completeness() {
    let library = populate_knowledge_library().expect("Failed to populate knowledge library");
    let tight_coupling = library.universal_patterns.get("tight_coupling")
    ...
    let has_metric_detection = tight_coupling.detection_methods.iter().any(|method| {
        matches!(method, DetectionMethod::MetricThreshold { .. })
    });
    assert!(has_metric_detection, "Should have metric-based detection methods");
    
    // Should have dependency analysis
    let has_dependency_analysis = tight_coupling.detection_methods.iter().any(|method| {
        matches!(method, DetectionMethod::DependencyAnalysis { .. })
    });
    assert!(has_dependency_analysis, "Should have dependency analysis methods");
...
fn test_dead_code_pattern_accuracy() {
    let library = populate_knowledge_library().expect("Failed to populate knowledge library");
    let dead_code = library.universal_patterns.get("dead_code")
    ...
    let has_static_analysis = dead_code.detection_methods.iter().any(|method| {
        matches!(method, DetectionMethod::StaticAnalysis { .. })
    });
    assert!(has_static_analysis, "Should have static analysis detection methods");
    
    // Should have AST pattern detection
    let has_ast_pattern = dead_code.detection_methods.iter().any(|method| {
        matches!(method, DetectionMethod::AstPattern { .. })
    });
...
fn test_code_duplication_pattern_coverage() {
    let library = populate_knowledge_library().expect("Failed to populate knowledge library");
    let code_dup = library.universal_patterns.get("code_duplication")
    ...
    let all_text = format!("{} {}", 
                          code_dup.definition.as_str(),
                          code_dup.symptoms.iter()
                              .map(|s| s.as_str())
                              .collect::<Vec<_>>()
                              .join(" "));
    
    // Check for clone type mentions (from actual detector implementation)
    assert!(all_text.to_lowercase().contains("identical") || 
...
fn test_detection_context_mappings() {
    ...
    let has_oo_patterns = library.detection_context.iter()
        .any(|(context, patterns)| {
            context.contains("object") && !patterns.is_empty()
        });
    assert!(has_oo_patterns, "Should have object-oriented pattern mappings");
    
    let has_architectural_patterns = library.detection_context.iter()
        .any(|(context, patterns)| {
            context.contains("architectural") && !patterns.is_empty()
        });
    assert!(has_architectural_patterns, "Should have architectural pattern mappings");
    
    // Validate that all referenced patterns exist
    for (context, pattern_ids) in &library.detection_context {
        for pattern_id in pattern_ids {
            assert!(library.universal_patterns.contains_key(pattern_id),
                    "Context '{}' references non-existent pattern '{}'", context, pattern_id);
        }
    }
...
fn test_solution_pattern_mappings() {
    ...
    assert!(library.solution_patterns.len() > 0, "Should have solution pattern mappings");
    
    // Should have common solution types
    let expected_solutions = ["Extract Class", "Dependency Injection", "Interface Abstraction"];
    ...
    let effort_mappings = library.solution_patterns.iter()
        .filter(|(key, _)| key.contains("effort"))
        .count();
    assert!(effort_mappings > 0, "Should have effort-level solution mappings");
...
fn test_compression_string_optimization() {
    ...
    for (pattern_id, pattern) in &library.universal_patterns {
        ...
        assert!(pattern.definition.len() > 0, 
                "Pattern '{}' should have a non-empty definition", pattern_id);
        
        // Symptoms should be compressed
        for (i, symptom) in pattern.symptoms.iter().enumerate() {
            assert!(symptom.len() > 0,
                    "Pattern '{}' symptom {} should not be empty", pattern_id, i);
        }
        
        // Solutions should have compressed implementation text
        for (i, solution) in pattern.solutions.iter().enumerate() {
            assert!(solution.implementation.len() > 0,
                    "Pattern '{}' solution {} should have implementation details", pattern_id, i);
        }
    }
...
fn test_knowledge_library_validation() {
    ...
    assert!(report.coverage_score > 0.5, 
            "Coverage score should be reasonable: {}", report.coverage_score);
    assert!(report.quality_score > 0.5,
            "Quality score should be reasonable: {}", report.quality_score);
    
    // Should not have critical issues
    let critical_issues = report.issues.iter()
        .filter(|issue| issue.severity == Severity::Critical)
        .count();
    assert_eq!(critical_issues, 0, "Should not have critical validation issues");
    
    // Should have actionable recommendations
    assert!(report.recommendations.len() > 0, "Should provide recommendations");
    
    println!("✓ Knowledge library passes validation with scores - Coverage: {:.2}, Quality: {:.2}",
...
fn test_enhanced_knowledge_population() {
    let library = populate_from_detector_analysis()
        .expect("Failed to populate enhanced knowledge library");
    ...
    let has_language_thresholds = god_object.detection_methods.iter().any(|method| {
        match method {
            DetectionMethod::MetricThreshold { metric_name, .. } => {
                metric_name.contains("rust") || metric_name.contains("python") || metric_name.contains("javascript")
            },
            _ => false,
        }
    });
    assert!(has_language_thresholds, "Should have language-specific detection thresholds");
...
fn test_library_metadata() {
    ...
    println!("  Supported languages: {:?}", library.metadata.supported_languages);
}
...
fn test_pattern_relationships() {
    ...
    for (pattern_id, pattern) in &library.universal_patterns {
        for related_id in &pattern.related_patterns {
            assert!(all_pattern_ids.contains(related_id),
    ...
    assert!(tight_coupling.related_patterns.len() > 0,
...
fn test_detection_methods_configuration() {
    ...
    for (pattern_id, pattern) in &library.universal_patterns {
        assert!(pattern.detection_methods.len() > 0,
                "Pattern '{}' should have at least one detection method", pattern_id);
...
fn test_ai_prompt_integration() {
    ...
    let prompt_content = format!(
        ...
        god_object.symptoms.iter()
            .map(|s| s.as_str())
            .collect::<Vec<_>>()
            .join(", "),
        god_object.solutions.iter()
            .map(|s| &s.title)
            .collect::<Vec<_>>()
            .join(", ")
    );
    
    assert!(prompt_content.len() > 200, "AI prompt content should be substantial");
    assert!(prompt_content.contains("God Object"), "Should contain pattern name");
    assert!(prompt_content.contains("Extract Class"), "Should contain solution information");
    
    println!("✓ Knowledge library is suitable for AI prompt generation");
    println!("  Sample prompt length: {} characters", prompt_content.len());
}
...
fn test_library_performance() {
    ...
    assert!(population_time.as_millis() < 1000, 
            "Population should complete in under 1 second, took: {:?}", population_time);
    assert!(validation_time.as_millis() < 500,
            "Validation should complete in under 500ms, took: {:?}", validation_time);
...
fn test_pattern_categorization() {
    ...
    assert!(category_counts.len() >= 3, "Should have patterns in multiple categories");
    
    // Should have some object-oriented patterns
    assert!(category_counts.contains_key(&AntiPatternCategory::ObjectOriented),
            "Should have object-oriented patterns");
    
    // Should have some architectural patterns  
    assert!(category_counts.contains_key(&AntiPatternCategory::Architectural),
...
fn test_end_to_end_integration() {
    ...
    let validator = KnowledgeValidator::default();
    let report = validator.validate_library(&library);
    assert!(report.is_valid || report.issues.iter().all(|i| i.severity != Severity::Critical));
    
    // 3. Test knowledge retrieval
    let god_object_patterns = library.get_patterns_by_category(&AntiPatternCategory::ObjectOriented);
    assert!(god_object_patterns.len() > 0, "Should find object-oriented patterns");
    ...
    if let Some(patterns) = context_results {
        assert!(patterns.len() > 0, "Should find patterns for context lookup");
    }
