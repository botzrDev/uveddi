...
impl DependencyAnalyzer {
    ...
    pub fn analyze_project(&mut self, project_path: &Path) -> Result<Vec<CircularDependency>, Box<dyn std::error::Error>> {
    ...
    fn build_dependency_graph(&mut self, project_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn calculate_severity(&self, modules: &[String]) -> CycleSeverity {
        ...
        let has_critical = modules.iter().any(|m| {
            critical_modules.iter().any(|&crit| m.starts_with(crit))
        });
    ...
    fn suggest_fix(&self, modules: &[String]) -> String {
        if modules.iter().any(|m| m.contains("analysis")) && modules.iter().any(|m| m.contains("database")) {
            "Create persistence interface to break Analysis ↔ Database dependency".to_string()
        } else if modules.iter().any(|m| m.contains("ast")) && modules.iter().any(|m| m.contains("analysis")) {
