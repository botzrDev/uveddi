...
"#;
        ...
        assert!(
            issues.iter().any(|issue| issue.description.contains("LargeStruct")),
            "Should detect LargeStruct as large class"
        );
        
        // Check severity is appropriate for a class with many methods and fields
        let issue = &issues[0];
        println!("DEBUG: Found issue with severity: '{}', description: '{}'", issue.severity, issue.description);
        assert!(
            issue.severity == "Low" || issue.severity == "Medium" || issue.severity == "High" || issue.severity == "Critical",
            "Should have appropriate severity for large class, got: '{}'", issue.severity
        );
        
        // Verify the description contains detailed metrics
        assert!(issue.description.contains("LargeStruct"), "Should mention the struct name");
        assert!(issue.description.contains("Methods:"), "Should include method count");
        assert!(issue.description.contains("Fields:"), "Should include field count");
        assert!(issue.description.contains("refactoring"), "Should include refactoring suggestions");
    }
...
"#;
        ...
        assert!(issues.is_empty(), "Should not detect issues for small, well-designed class");
    }
...
"#;
        ...
        if let Ok(tree) = parsed_file.tree.as_ref().ok_or("No tree") {
            let logical_loc = detector.calculate_logical_loc(&tree.root_node(), rust_code.as_bytes());
            // Should count actual code lines, not comments or blank lines
            assert!(logical_loc > 0, "Should calculate logical LOC > 0");
            assert!(logical_loc < rust_code.lines().count() as u32, "Logical LOC should be less than total lines");
        }
    }
    ...
    fn test_language_specific_thresholds() {
        ...
        assert!(js_thresholds.max_coupling >= python_thresholds.max_coupling,
...
"#;
    ...
    fn test_description_generation() {
        ...
        assert!(description.contains("refactoring"), "Should include refactoring suggestions");
    }
...
"#;
        ...
        if let Ok(tree) = parsed_file.tree.as_ref().ok_or("No tree") {
            ...
            assert!(snippet.contains("TestStruct"), "Snippet should contain struct name");
        }
    }
}
