...
impl DiagramDependencyTracker {
    ...
    pub async fn add_diagram_dependencies(
        ...
        debug!("Added dependencies for diagram: {} (files: {})", diagram_id, source_files.len());
        Ok(())
    }
    ...
    pub async fn get_affected_diagrams(&self, changeset: &ChangeSet) -> Result<HashSet<String>> {
        ...
        let propagated_diagrams = self.propagate_diagram_dependencies(&affected_diagrams).await?;
        affected_diagrams.extend(propagated_diagrams);
        
        info!("Found {} diagrams affected by changeset", affected_diagrams.len());
        Ok(affected_diagrams)
    }
    
    /// Gets diagrams that need regeneration based on changes
    pub async fn get_diagrams_for_regeneration(&self, changeset: &ChangeSet) -> Result<Vec<String>> {
        ...
        info!("Identified {} diagrams for regeneration", diagrams_to_regenerate.len());
    ...
    pub async fn add_diagram_dependency(&self, from_diagram: &str, to_diagram: &str) -> Result<()> {
        ...
        if let Some(dependency) = deps.get_mut(from_diagram) {
            dependency.diagram_dependencies.insert(to_diagram.to_string());
            dependency.last_updated = chrono::Utc::now();
        }
        
        debug!("Added diagram dependency: {} -> {}", from_diagram, to_diagram);
        Ok(())
    }
    
    /// Gets all dependencies for a diagram
    pub async fn get_diagram_dependencies(&self, diagram_id: &str) -> Result<Option<DiagramDependency>> {
        let deps = self.diagram_dependencies.read().await;
        Ok(deps.get(diagram_id).cloned())
    }
    
    /// Updates dependency strength based on usage patterns
    pub async fn update_dependency_strength(&self, diagram_id: &str, new_strength: f64) -> Result<()> {
        let mut deps = self.diagram_dependencies.write().await;
        if let Some(dependency) = deps.get_mut(diagram_id) {
            dependency.dependency_strength = new_strength.clamp(0.0, 1.0);
            dependency.last_updated = chrono::Utc::now();
            debug!("Updated dependency strength for {}: {:.2}", diagram_id, new_strength);
        }
        Ok(())
    }
    ...
    pub async fn analyze_dependency_patterns(&self) -> Result<DependencyAnalysis> {
        ...
        for (file, diagrams) in file_map.iter() {
            if diagrams.len() > 5 { // Threshold for "hot" files
    ...
    async fn propagate_diagram_dependencies(&self, initial_diagrams: &HashSet<String>) -> Result<HashSet<String>> {
