...
impl EnterpriseMetricsCollector {
    ...
    pub async fn record_pipeline_measurement(&self, latency: Duration, success: bool) {
        ...
        if latency_ms < 70.0 {
            let under_target_count = (metrics.under_target_percentage / 100.0 * (metrics.total_analyses - 1) as f64) + 1.0;
            metrics.under_target_percentage = (under_target_count / metrics.total_analyses as f64) * 100.0;
        } else {
            let under_target_count = metrics.under_target_percentage / 100.0 * (metrics.total_analyses - 1) as f64;
            metrics.under_target_percentage = (under_target_count / metrics.total_analyses as f64) * 100.0;
        }
    ...
    pub async fn record_memory_measurement(&self, current_usage_bytes: u64, allocation_delta: i64) {
        ...
        if metrics.average_usage_bytes == 0 {
            ...
            metrics.average_usage_bytes = 
                ((metrics.average_usage_bytes as f64 * 0.95) + (current_usage_bytes as f64 * 0.05)) as u64;
        }
    ...
    pub async fn record_ast_parsing_measurement(&self, language: &str, parse_time: Duration, 
                                               ast_size_nodes: u64, success: bool) {
        ...
        if !success {
            metrics.parse_error_rate = 
                (metrics.parse_error_rate * (count - 1.0) + 1.0) / count;
        } else {
            metrics.parse_error_rate = 
                (metrics.parse_error_rate * (count - 1.0)) / count;
        }
        
        // Update language-specific metrics
        let lang_metrics = metrics.language_specific_metrics.entry(language.to_string())
        ...
        lang_metrics.average_ast_size_nodes = 
            ((lang_metrics.average_ast_size_nodes * (lang_metrics.files_parsed - 1)) + ast_size_nodes) 
    ...
    pub async fn record_diagram_generation_measurement(&self, diagram_type: &str, 
        ...
        if success {
            ...
            let type_metrics = metrics.diagram_types.entry(diagram_type.to_string())
            ...
            type_metrics.average_time_ms = 
                (type_metrics.average_time_ms * (type_count - 1.0) + generation_time_ms) / type_count;
    ...
    pub async fn record_cache_measurement(&self, cache_layer: &str, hit: bool, 
                                        lookup_time: Duration, cache_size_bytes: u64) {
        ...
        if hit {
            metrics.overall_hit_rate = 
                (metrics.overall_hit_rate * (count - 1.0) + 100.0) / count;
        } else {
            metrics.overall_hit_rate = 
                (metrics.overall_hit_rate * (count - 1.0) + 0.0) / count;
        }
        
        // Update average lookup time
        metrics.average_lookup_time_microseconds = 
            (metrics.average_lookup_time_microseconds * (count - 1.0) + lookup_time_microseconds) / count;
        ...
        let layer_metrics = metrics.cache_layers.entry(cache_layer.to_string())
        ...
        layer_metrics.average_lookup_time_microseconds = 
            (layer_metrics.average_lookup_time_microseconds * (layer_count - 1.0) + lookup_time_microseconds) / layer_count;
        
        metrics.last_updated = Utc::now();
    }

    /// Record incremental analysis measurement
    pub async fn record_incremental_analysis_measurement(&self, full_analysis_time: Duration,
    ...
    pub async fn take_snapshot(&self) -> MeasurementSnapshot {
        ...
        let overall_score = self.calculate_overall_score(&pipeline, &memory, &ast, &diagram, &cache, &incremental);
        ...
        // Store in history
        self.measurement_history.write().await.push(snapshot.clone());
    ...
    pub async fn detect_regressions(&self) -> RegressionAnalysis {
        ...
        if self.check_pipeline_regression(&baseline.pipeline_baseline, &current.pipeline, &mut regression_details) {
            has_regression = true;
        }
        
        // Check memory metrics
        if self.check_memory_regression(&baseline.memory_baseline, &current.memory, &mut regression_details) {
            has_regression = true;
        }
        
        // Check AST metrics
        if self.check_ast_regression(&baseline.ast_baseline, &current.ast, &mut regression_details) {
            has_regression = true;
        }
        
        // Check diagram metrics
        if self.check_diagram_regression(&baseline.diagram_baseline, &current.diagram, &mut regression_details) {
            has_regression = true;
        }
        
        // Check cache metrics
        if self.check_cache_regression(&baseline.cache_baseline, &current.cache, &mut regression_details) {
            has_regression = true;
        }
        
        // Check incremental metrics
        if self.check_incremental_regression(&baseline.incremental_baseline, &current.incremental, &mut regression_details) {
            has_regression = true;
        }
        
        let overall_change = ((current.overall_score - baseline.pipeline_baseline.average_latency_ms) 
            / baseline.pipeline_baseline.average_latency_ms) * 100.0;
        ...
        let recommendation = self.generate_regression_recommendation(&regression_details, &severity);
    ...
    fn calculate_overall_score(&self, pipeline: &PipelineMetrics, memory: &MemoryMetrics,
                              ast: &AstParsingMetrics, diagram: &DiagramGenerationMetrics,
                              cache: &CacheMetrics, incremental: &IncrementalAnalysisMetrics) -> f64 {
        // Weighted composite score (0-100)
        let pipeline_score = if pipeline.average_latency_ms < 70.0 { 100.0 } else { 70.0 / pipeline.average_latency_ms * 100.0 };
        let memory_score = if memory.current_usage_bytes < 8 * 1024 * 1024 * 1024 { 100.0 } else { 50.0 };
        let ast_score = (100.0 - ast.parse_error_rate).max(0.0);
        let diagram_score = if diagram.generation_rate_per_minute >= 1000.0 { 100.0 } else { diagram.generation_rate_per_minute / 10.0 };
        let cache_score = cache.overall_hit_rate;
        let incremental_score = incremental.average_improvement_percentage.max(0.0).min(100.0);
        
        // Weighted average
        (pipeline_score * 0.25 + memory_score * 0.20 + ast_score * 0.15 + 
         diagram_score * 0.15 + cache_score * 0.15 + incremental_score * 0.10)
    }

    fn check_pipeline_regression(&self, baseline: &PipelineMetrics, current: &PipelineMetrics, 
                                details: &mut Vec<RegressionDetail>) -> bool {
        let latency_change = ((current.average_latency_ms - baseline.average_latency_ms) 
            / baseline.average_latency_ms) * 100.0;
        
        if latency_change > 10.0 { // 10% threshold
            details.push(RegressionDetail {
                ...
                impact_level: if latency_change > 30.0 { RegressionImpact::Critical } 
                             else if latency_change > 20.0 { RegressionImpact::High }
                             else { RegressionImpact::Medium },
            });
            return true;
        }
        false
    ...
    fn check_memory_regression(&self, baseline: &MemoryMetrics, current: &MemoryMetrics, 
                              details: &mut Vec<RegressionDetail>) -> bool {
        let memory_change = ((current.peak_usage_bytes as f64 - baseline.peak_usage_bytes as f64) 
            / baseline.peak_usage_bytes as f64) * 100.0;
        
        if memory_change > 15.0 { // 15% threshold
            details.push(RegressionDetail {
                ...
                threshold_exceeded: true,
                impact_level: if memory_change > 50.0 { RegressionImpact::Critical } 
                             else if memory_change > 30.0 { RegressionImpact::High }
                             else { RegressionImpact::Medium },
            });
            return true;
        }
    ...
    fn check_ast_regression(&self, baseline: &AstParsingMetrics, current: &AstParsingMetrics, 
                           details: &mut Vec<RegressionDetail>) -> bool {
        let parse_time_change = ((current.average_parse_time_ms - baseline.average_parse_time_ms) 
            / baseline.average_parse_time_ms) * 100.0;
        
        if parse_time_change > 20.0 { // 20% threshold
            details.push(RegressionDetail {
                ...
                threshold_exceeded: true,
                impact_level: if parse_time_change > 50.0 { RegressionImpact::Critical } 
                             else { RegressionImpact::Medium },
            });
            return true;
        }
    ...
    fn check_diagram_regression(&self, baseline: &DiagramGenerationMetrics, current: &DiagramGenerationMetrics, 
                               details: &mut Vec<RegressionDetail>) -> bool {
        let rate_change = ((current.generation_rate_per_minute - baseline.generation_rate_per_minute) 
            / baseline.generation_rate_per_minute) * 100.0;
        
        if rate_change < -10.0 { // 10% decrease threshold
            details.push(RegressionDetail {
                ...
                threshold_exceeded: true,
                impact_level: if rate_change < -30.0 { RegressionImpact::Critical } 
                             else { RegressionImpact::Medium },
            });
            return true;
        }
    ...
    fn check_cache_regression(&self, baseline: &CacheMetrics, current: &CacheMetrics, 
                             details: &mut Vec<RegressionDetail>) -> bool {
        let hit_rate_change = current.overall_hit_rate - baseline.overall_hit_rate;
        
        if hit_rate_change < -5.0 { // 5% decrease threshold
            details.push(RegressionDetail {
                ...
                threshold_exceeded: true,
                impact_level: if hit_rate_change < -15.0 { RegressionImpact::High } 
                             else { RegressionImpact::Medium },
            });
            return true;
        }
    ...
    fn check_incremental_regression(&self, baseline: &IncrementalAnalysisMetrics, current: &IncrementalAnalysisMetrics, 
                                   details: &mut Vec<RegressionDetail>) -> bool {
        let improvement_change = current.average_improvement_percentage - baseline.average_improvement_percentage;
        
        if improvement_change < -10.0 { // 10% decrease threshold
            details.push(RegressionDetail {
                ...
                threshold_exceeded: true,
                impact_level: if improvement_change < -25.0 { RegressionImpact::Critical } 
                             else { RegressionImpact::Medium },
            });
            return true;
        }
    ...
    fn generate_regression_recommendation(&self, details: &[RegressionDetail], severity: &RegressionSeverity) -> String {
        if details.is_empty() {
