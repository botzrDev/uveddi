...
impl PostgreSqlProvider {
    ...
    pub fn new(config: DatabaseConfig) -> Result<Self> {
        let write_pool = Arc::new(PostgreSqlConnectionPool::new(&config.connection_string, &config)?);
        ...
        let read_pool = if config.read_connection_strings.is_empty() {
            ...
            Arc::new(PostgreSqlConnectionPool::new(&config.read_connection_strings[0], &read_config)?)
        };
        ...
        self.execute_with_metrics(|| {
            ...
            for issue in issues {
                ...
                let csv_row = format!("{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}\n",
                    ...
                    issue.column_number.map(|c| c.to_string()).unwrap_or_default(),
                    ...
                    issue.code_snippet.as_ref().map(|s| csv_escape(s)).unwrap_or_default(),
                    sanitized_ai_explanation.as_ref().map(|s| csv_escape(s)).unwrap_or_default(),
                );
...
impl PostgreSqlConnectionPool {
    ...
    async fn get_connection(&self) -> Result<PostgreSqlConnection> {
        let _permit = self.semaphore.clone().acquire_owned().await
            .map_err(|e| UveddiError::database_error_msg(&format!("Failed to acquire PostgreSQL connection permit: {}", e)))?;
    ...
    async fn get_stats(&self) -> Result<PoolStats> {
        Ok(PoolStats {
            ...
            active_connections: self.config.max_connections as usize - self.semaphore.available_permits(),
            pending_requests: 0,
...
impl PostgreSqlConnection {
    ...
    fn query_one(&self, _stmt: &PostgreSqlStatement, _params: &[&(dyn std::fmt::Debug)]) -> Result<PostgreSqlRow> {
        todo!("PostgreSQL query_one implementation")
    }
    
    fn query_opt(&self, _stmt: &PostgreSqlStatement, _params: &[&(dyn std::fmt::Debug)]) -> Result<Option<PostgreSqlRow>> {
