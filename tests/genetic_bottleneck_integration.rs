...
async fn test_genetic_bottleneck_detector_basic() -> Result<()> {
    ...
    println!("   - Bottlenecks identified: {}", analysis.identified_bottlenecks.len());
    println!("   - Recommendations: {}", analysis.optimization_recommendations.len());
...
async fn test_cpu_bottleneck_detection() -> Result<()> {
    ...
    assert!(analysis.performance_score < 90.0, "Should detect CPU bottleneck impact");
    
    println!("✅ CPU bottleneck detection test passed");
    println!("   - Performance score: {:.1} (expected < 90)", analysis.performance_score);
    println!("   - Analysis duration: {}ms", analysis.analysis_metadata.analysis_duration_ms);
...
async fn test_memory_bottleneck_detection() -> Result<()> {
    ...
    assert!(analysis.overall_confidence > 0.5, "Should have reasonable confidence in detection");
...
async fn test_regression_detector_integration() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        detector.record_metric("test_latency", 100.0 + i as f64, metadata).await?;
    }

    // Test genetic bottleneck analysis
    let analysis = detector.analyze_bottlenecks_genetic("test_latency", 24).await?;

    if let Some(analysis) = analysis {
        assert!(analysis.analysis_metadata.metrics_analyzed > 0);
        println!("✅ Regression detector integration test passed");
        println!("   - Metrics analyzed: {}", analysis.analysis_metadata.metrics_analyzed);
        println!("   - Final generation: {}", analysis.analysis_metadata.final_generation);
    } else {
...
async fn test_comprehensive_analysis() -> Result<()> {
    ...
    for i in 0..8 {
        ...
        metadata.insert("cpu_usage".to_string(), format!("{:.2}", 0.3 + i as f64 * 0.05));
        metadata.insert("memory_usage".to_string(), format!("{:.2}", 0.4 + i as f64 * 0.03));
        
        detector.record_metric("service_latency", 150.0 + i as f64 * 10.0, metadata).await?;
    }

    // Perform comprehensive analysis with a regression scenario
    let result = detector.comprehensive_performance_analysis(
        "service_latency", 
    ...
    println!("   - Recommendations: {}", result.comprehensive_recommendations.len());
...
async fn test_genetic_algorithm_convergence() -> Result<()> {
    ...
    assert!(analysis.performance_score > 80.0, "Stable data should have good performance score");
    
    println!("✅ Genetic algorithm convergence test passed");
    println!("   - Performance score: {:.1}", analysis.performance_score);
    println!("   - Convergence achieved: {}", analysis.convergence_achieved);
    println!("   - Population size: {}", analysis.analysis_metadata.population_size);
...
async fn test_uv249_acceptance_criteria() -> Result<()> {
    ...
    let _analysis = detector.evolve_bottleneck_detection(&performance_data).await?;
    let duration = start_time.elapsed();
    assert!(duration.as_secs() < 30); // Should complete within reasonable time
    println!("✅ Performance overhead validated ({}ms)", duration.as_millis());
