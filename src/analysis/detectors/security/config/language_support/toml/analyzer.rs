...
use super::{TomlCredentialChecker, TomlDependencyChecker, TomlBuildChecker};
    ...
    fn analyze(&self, content: &str) -> Result<Vec<ConfigIssue>, AnalysisError> {
        ...
        match content.parse::<TomlValue>() {
            Ok(toml_value) => {
                ...
                issues.extend(self.credential_checker.check_database_credentials(&toml_value));

                // Check dependency security
                issues.extend(self.dependency_checker.check_dependency_security(&toml_value)?);
...
impl TomlAnalyzer {
    ...
    fn check_formatter_config(&self, formatter_config: &toml::value::Table, tool_name: &str) -> Vec<ConfigIssue> {
        let mut issues = Vec::new();
        ...
        if let Some(TomlValue::Integer(line_length)) = formatter_config.get("line-length") {
            if *line_length > 200 {
                issues.push(utils::create_config_issue(
                    ...
                    format!("{} configured with very long line length: {}", tool_name, line_length),
                    None,
...
"#;
        ...
        assert!(issues.iter().any(|i| i.title.contains("Very Long Line Length")));
    }
