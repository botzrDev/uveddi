...
impl<'a> RustImplAnalyzer<'a> {
    ...
    pub fn count_impl_methods(
        &self,
        parsed_file: &ParsedFile,
    ) -> Result<HashMap<String, usize>, AnalysisError> {
        let mut impl_method_counts: HashMap<String, usize> = HashMap::new();
        let source = parsed_file.source.as_bytes();
        let tree = parsed_file
            .tree
            .as_ref()
            .ok_or_else(|| ErrorHelpers::ast_error("impl analysis"))?;
        ...
        while let Some(mat) = matches.next() {
                ...
                (mat.captures.first(), mat.captures.get(1))
            {
                ...
                if let Ok(name) = name_node.utf8_text(source) {
                    let method_count =
                        MetricsCalculator::calculate_method_count(parsed_file, body_node)?;
