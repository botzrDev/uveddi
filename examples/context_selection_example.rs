...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Detected Patterns: {} issues found",
        analysis_context.detected_patterns.len()
    );
    ...
    println!(
        "   Strategy Used: {:?}",
        selected_context.metadata.strategy_used
    );
    println!(
        "   Selection Time: {}ms",
        selected_context.metadata.selection_time_ms
    );
    println!(
        "   Patterns Considered: {}",
        selected_context.metadata.total_candidates
    );
    println!(
        "   Patterns Selected: {}",
        selected_context.metadata.selected_count
    );
    println!(
        "   Average Relevance: {:.3}",
        selected_context.metadata.average_relevance
    );
    println!(
        "   Token Utilization: {:.1}% ({}/{} tokens)",
    ...
    for (i, scored_pattern) in selected_context.patterns.iter().enumerate() {
        ...
        println!(
            "      Match Type: {:?}",
            scored_pattern.candidate.match_type
        );
    ...
    analytics.add_feedback(
        "god_object".to_string(),
        5,
        "Excellent detection!".to_string(),
    );
    analytics.add_feedback(
        "tight_coupling".to_string(),
        3,
        "Somewhat helpful".to_string(),
    );

    println!("   Usage scores updated:");
    println!(
        "     - god_object: {:.3}",
        analytics.get_usage_score("god_object")
    );
    println!(
        "     - tight_coupling: {:.3}",
        analytics.get_usage_score("tight_coupling")
    );
    println!(
        "     - new_pattern: {:.3} (neutral)",
        analytics.get_usage_score("new_pattern")
    );
...
fn create_sample_enhanced_patterns() -> HashMap<String, EnhancedPattern> {
        ...
        universal: PatternKnowledge {
            ...
            solutions: vec![SolutionPattern {
                ...
                implementation: CompressedString::new(
                    "Break down the large class into smaller, focused components",
                ),
                examples: vec![],
    ...
    let tight_coupling = EnhancedPattern {
        universal: PatternKnowledge {
            ...
            definition: CompressedString::new(
                "Classes or modules that are overly dependent on each other",
            ),
            symptoms: vec![
