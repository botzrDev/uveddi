...
use super::traits::{DetectorScheduler as DetectorSchedulerTrait, AstProvider, AnalysisAggregator, ConfigurationService, PluginManagerHandle as PluginManagerHandleTrait};
use super::plugin_manager::PluginManagerHandle;
use crate::analysis::{AnalysisDetector, detectors::cycle::CycleDetector};
...
impl DetectorScheduler {
    ...
    async fn analyze_file(&self, file_path: &Path) -> Result<Vec<ArchitecturalIssue>, UveddiError> {
        ...
        if let Some(ref plugin_manager) = self.plugin_manager {
            if let Err(e) = self.run_plugin_detectors(plugin_manager, file_path, ast).await {
                warn!("Error running plugin detectors on {}: {}", file_path.display(), e);
            }
        }
            ...
            match PluginManagerHandleTrait::execute_plugin(
                ...
                Ok(plugin_issues) => {
                    if !plugin_issues.is_empty() {
                        info!("Plugin {} found {} issues in {}", 
                              plugin_id, plugin_issues.len(), file_path.display());
                        self.aggregator.record_findings(plugin_issues);
                    }
                }
                Err(e) => {
                    warn!("Plugin {} failed on {}: {}", plugin_id, file_path.display(), e);
                }
            }
        }
...
impl DetectorSchedulerTrait for DetectorScheduler {
    ...
    async fn schedule_directory(&self, dir_path: &Path) -> Result<Vec<ArchitecturalIssue>, UveddiError> {
        ...
        info!("Completed directory analysis: {} files processed, {} issues found", 
              files_processed, all_issues.len());
        
        Ok(all_issues)
    }
    
    async fn schedule_graph_analysis(&self, graph: &LocalDependencyGraph) -> Result<Vec<ArchitecturalIssue>, UveddiError> {
        info!("Running graph-level analysis on dependency graph with {} nodes", graph.node_count());
        ...
        info!("Graph analysis completed: {} total graph-level issues found", graph_issues.len());
...
mod tests {
    use super::*;
    use crate::analysis::components::{ConfigurationService, AstProviderImpl, AnalysisAggregator};
    use crate::analysis::components::traits::{ConfigurationService as ConfigurationServiceTrait, AstProvider, AnalysisAggregator as AnalysisAggregatorTrait};
    ...
    async fn create_test_scheduler() -> DetectorScheduler {
        let config_service = Arc::new(ConfigurationService::new()) as Arc<dyn ConfigurationServiceTrait>;
        ...
        let detectors: Vec<Box<dyn AnalysisDetector + Send + Sync>> = vec![
            Box::new(GodObjectDetector::new(10, 15)),
        ];
