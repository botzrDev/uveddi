...
impl DatasetEntry {
    fn new(id: u64, complexity: &str) -> Self {
        ...
        match complexity {
            ...
            "complex" => {
                for i in 0..100 {
                    metadata.insert(format!("complex_key_{}", i), format!("complex_value_{}", i * 2));
...
impl DatasetGenerator {
    ...
    fn generate_entries(&mut self, count: usize, complexity_mix: &[(&str, f32)]) {
        ...
        for (complexity, ratio) in complexity_mix {
            ...
            self.complexity_distribution.insert(complexity.to_string(), entry_count);
    ...
    fn filter_entries_by_tag(&self, tag_pattern: &str) -> Vec<&DatasetEntry> {
        self.entries
            .iter()
            .filter(|entry| {
                entry.tags.iter().any(|tag| tag.contains(tag_pattern))
            })
            .collect()
    }
...
fn bench_transformation_pipelines(c: &mut Criterion) {
    ...
    let mut generator = DatasetGenerator::new();
    generator.generate_entries(1000, &[
        ("simple", 0.2),
        ("medium", 0.5),
        ("complex", 0.3),
    ]);
