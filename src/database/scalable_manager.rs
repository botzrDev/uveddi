...
impl ScalableDatabase {
    ...
    pub async fn new(config: DatabaseConfig) -> Result<Self> {
        ...
        let write_provider: Arc<dyn DatabaseProvider> =
            Arc::from(create_database_provider(&config)?);
        ...
        if config.read_connection_strings.is_empty() {
            ...
            for read_conn_str in &config.read_connection_strings {
                ...
                let read_provider: Arc<dyn DatabaseProvider> =
                    Arc::from(create_database_provider(&read_config)?);
    ...
    pub async fn get_or_create_project_id(&self, project_path: &Path) -> Result<i64> {
        self.write_query(
            |provider| async move { provider.get_or_create_project_id(project_path).await },
        )
        .await
    }

    pub async fn create_analysis_run(&self, project_path: &Path) -> Result<AnalysisRun> {
        self.write_query(|provider| async move { provider.create_analysis_run(project_path).await })
            .await
    }

    pub async fn update_analysis_run(&self, run: &AnalysisRun) -> Result<()> {
        self.write_query(|provider| async move { provider.update_analysis_run(run).await })
            .await
    }

    pub async fn store_anti_pattern_types_batch(
        &self,
    ...
    pub async fn get_analysis_run(&self, run_id: i64) -> Result<Option<AnalysisRun>> {
        self.read_query(|provider| async move { provider.get_analysis_run(run_id).await })
            .await
    }

    pub async fn get_latest_analysis_run(&self) -> Result<Option<AnalysisRun>> {
        self.read_query(|provider| async move { provider.get_latest_analysis_run().await })
            .await
    }

    pub async fn get_recent_analysis_runs(&self, limit: u32) -> Result<Vec<AnalysisRun>> {
        self.read_query(|provider| async move { provider.get_recent_analysis_runs(limit).await })
            .await
    }

    pub async fn get_issues_for_run(&self, run_id: i64) -> Result<Vec<ArchitecturalIssue>> {
        self.read_query(|provider| async move { provider.get_issues_for_run(run_id).await })
            .await
    }

    pub async fn get_dependencies_for_run(&self, run_id: i64) -> Result<Vec<Dependency>> {
        self.read_query(|provider| async move { provider.get_dependencies_for_run(run_id).await })
            .await
    }
    ...
    pub async fn get_issues_with_types_for_run(
        ...
        self.read_query(
            |provider| async move { provider.get_issues_with_types_for_run(run_id).await },
        )
        .await
    }

    pub async fn get_analysis_stats(&self, run_id: i64) -> Result<AnalysisStats> {
        self.read_query(|provider| async move { provider.get_analysis_stats(run_id).await })
            .await
    }
    ...
    pub async fn get_issues_paginated(
        ...
        self.read_query(|provider| async move {
            provider
                .get_issues_paginated(run_id, offset, limit, severity_filter, detector_filter)
                .await
        })
        .await
    }
    ...
    pub async fn get_all_anti_pattern_types(&self) -> Result<Vec<AntiPatternType>> {
        self.read_query(|provider| async move { provider.get_all_anti_pattern_types().await })
            .await
    }

    pub async fn get_project_path(&self, project_id: i64) -> Result<String> {
        self.read_query(|provider| async move { provider.get_project_path(project_id).await })
            .await
    }
...
impl ReadLoadBalancer {
    ...
    fn get_stats(&self) -> LoadBalancerStats {
        let states = self.provider_states.lock().unwrap();
        let total_requests = states
            .iter()
            .map(|s| s.success_count + s.failure_count)
            .sum();

        let healthy_providers = states.iter().filter(|s| s.is_healthy).count();
