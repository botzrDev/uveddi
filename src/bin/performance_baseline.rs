...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match analyzer.analyze_rendering_performance().await {
        Ok(baseline) => {
            ...
            println!("  - P50 (median): {:.2}ms", baseline.percentile_analysis.p50_ms);
            println!("  - P95: {:.2}ms", baseline.percentile_analysis.p95_ms);
            println!("  - P99: {:.2}ms", baseline.percentile_analysis.p99_ms);
            println!("  - Cache hit rate: {:.1}%", baseline.cache_performance.hit_rate * 100.0);
            println!("  - Critical bottlenecks: {}", 
                baseline.bottlenecks.iter()
                    .filter(|b| matches!(b.impact_level, uveddi::analysis::performance::BottleneckSeverity::Critical))
                    .count());
