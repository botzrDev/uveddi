...
impl GodObjectDetector {
    ...
    fn is_generated_file(&self, file_path: &str, source: &str) -> Option<DetectedPattern> {
        ...
        let generated_markers = [
            "auto-generated", "auto generated", "DO NOT EDIT", "Code generated",
            "This file was automatically generated", "<auto-generated",
        ];
    ...
    fn analyze_imports(&self, parsed_file: &ParsedFile) -> Result<HashSet<String>, AnalysisError> {
        ...
        let tree = parsed_file.tree.as_ref()
            .ok_or_else(|| AnalysisError::AnalysisError(CoreAnalysisError::AntiPatternDetectionError("AST tree missing".to_string())))?;
        ...
        let query = Query::new(&language, query_str)
            .map_err(|e| AnalysisError::AnalysisError(CoreAnalysisError::QueryError(e.to_string())))?;
    ...
    fn extract_module_name(&self, import_text: &str, language: SourceLanguage) -> String {
        match language {
            SourceLanguage::Rust => {
                import_text.split("::").next().unwrap_or(import_text).to_string()
            },
    ...
    fn detect_builder_pattern(&self, parsed_file: &ParsedFile, class_node: Node, class_name: &str) -> Option<DetectedPattern> {
    ...
    fn detect_dto_pattern(&self, parsed_file: &ParsedFile, class_node: Node, method_count: usize, field_count: usize, frameworks: &HashSet<String>) -> Option<DetectedPattern> {
        if field_count == 0 {
            return None;
        }
        ...
        if field_ratio > 0.7 {
            ...
            for framework in frameworks {
                if ["serde", "pydantic", "dataclass"].iter().any(|f| framework.contains(f)) {
    ...
    fn calculate_lcom4(&self, parsed_file: &ParsedFile, class_node: Node) -> Result<u32, AnalysisError> {
    ...
    fn analyze_behavioral_complexity(&self, parsed_file: &ParsedFile, class_node: Node) -> Result<(usize, usize), AnalysisError> {
    ...
    fn score_severity(&self, 
        ...
        let method_threshold = self.config.method_thresholds.get(&language).copied().unwrap_or(10);
        let field_threshold = self.config.field_thresholds.get(&language).copied().unwrap_or(8);
            ...
            if self.config.recognize_patterns {
                ...
                if excluded_pattern.is_none() {
                    if let Some(pattern) = self.detect_dto_pattern(
                        parsed_file, container_node, method_count, field_count, &detected_frameworks
                    ) {
                        excluded_pattern = Some(pattern);
                    }
                }
                ...
                if excluded_pattern.is_none() && !detected_frameworks.is_empty() {
                    for framework in &detected_frameworks {
                        if ["django", "flask", "fastapi", "express", "react", "axum", "rocket"].contains(&framework.as_str()) {
    ...
    fn analyze_rust(
        ...
        if let Some(generated_pattern) = self.is_generated_file(
            ...
            debug!("Excluding Rust file as generated code: {:?}", generated_pattern);
            return Ok(issues);
        }
                            ...
                            if let Some(struct_node) = parent.next_sibling() {
                                if let Ok(struct_text) = struct_node.utf8_text(source) {
                                    if struct_text.starts_with("struct") {
                                        ...
                                        derive_attributes.entry(struct_name)
        ...
        for mat in struct_cursor.matches(&struct_query, root_node, source) {
                ...
                (mat.captures.first(), mat.captures.get(1))
            {
                ...
                if let Ok(name) = name_node.utf8_text(source) {
                    ...
                    let language = parsed_file.language;
                    let method_threshold = self.config.method_thresholds.get(&language).copied().unwrap_or(30);
                    let field_threshold = self.config.field_thresholds.get(&language).copied().unwrap_or(20);
