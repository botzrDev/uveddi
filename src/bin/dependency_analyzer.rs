...
impl DependencyGraph {
    ...
    fn add_dependency(&mut self, from: String, to: String) {
        self.modules.entry(from).or_insert_with(HashSet::new).insert(to);
    }
...
fn add_known_dependencies(graph: &mut DependencyGraph) {
    ...
    graph.add_dependency("infrastructure::database".to_string(), "core::interfaces::persistence".to_string());
    
    // Analysis depends on core interfaces (not direct database)
    graph.add_dependency("analysis::engine".to_string(), "core::interfaces::persistence".to_string());
    graph.add_dependency("analysis::engine".to_string(), "core::interfaces::events".to_string());
    
    // Services depend on analysis (but not database directly)
    graph.add_dependency("analysis::services".to_string(), "analysis::engine".to_string());
    graph.add_dependency("analysis::services".to_string(), "core::interfaces::events".to_string());
    
    // Some remaining cycles in legacy code (to be resolved in Phase 2-3)
    graph.add_dependency("config::manager".to_string(), "analysis::detectors".to_string());
    graph.add_dependency("analysis::detectors".to_string(), "config::manager".to_string());
    
    // Example of other cycles that might exist
    graph.add_dependency("security::scanner".to_string(), "analysis::engine".to_string());
    graph.add_dependency("analysis::engine".to_string(), "security::auditor".to_string());
    graph.add_dependency("security::auditor".to_string(), "security::scanner".to_string());
}
...
fn show_resolved_dependencies() {
    ...
    println!("      After:  analysis::engine → core::interfaces::persistence ← infrastructure::database");
