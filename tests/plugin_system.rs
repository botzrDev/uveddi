...
mod comprehensive_wasm_tests {
    ...
    async fn test_plugin_discovery_and_loading() {
        ...
        for manifest in manifests {
            let binary = create_minimal_wasm_binary();
            let plugin_id = registry.register_plugin(manifest.clone(), binary).await.unwrap();
    ...
    async fn test_concurrent_plugin_operations() {
        ...
        for i in 0..5 {
            let manifest = create_test_manifest_with_name(&format!("concurrent-plugin-{}", i), "1.0.0");
            let binary = create_minimal_wasm_binary();
    ...
    async fn test_plugin_isolation() {
        ...
        let plugin1_id = registry.register_plugin(
            plugin1_manifest, 
            create_minimal_wasm_binary()
        ).await.unwrap();
        
        let plugin2_id = registry.register_plugin(
            plugin2_manifest, 
            create_minimal_wasm_binary()
        ).await.unwrap();
    ...
    async fn test_code_signing_verification() {
        ...
        let result = verifier.verify_plugin(&binary, &wrong_manifest, &policy).await;
    ...
    async fn test_plugin_startup_performance() {
        ...
        assert!(startup_duration.as_millis() < 1000, 
               "Engine startup took too long: {:?}", startup_duration);
        ...
        assert!(registry_creation_duration.as_millis() < 100,
               "Registry creation took too long: {:?}", registry_creation_duration);
    }
    ...
    async fn test_memory_usage_patterns() {
        ...
        for i in 0..10 {
            let manifest = create_test_manifest_with_name(&format!("memory-test-plugin-{}", i), "1.0.0");
            let binary = create_minimal_wasm_binary();
            let _plugin_id = registry.register_plugin(manifest, binary).await.unwrap();
        }
        ...
        assert!(serialized.len() < 10 * 1024 * 1024, // Less than 10MB for test data
               "Serialized AST is too large: {} bytes", serialized.len());
    }
    ...
    async fn benchmark_plugin_operations() {
        ...
        let start = Instant::now();
        for i in 0..100 {
            let manifest = create_test_manifest_with_name(&format!("benchmark-plugin-{}", i), "1.0.0");
            let binary = create_minimal_wasm_binary();
            let _plugin_id = registry.register_plugin(manifest, binary).await.unwrap();
        }
        ...
        assert!(registration_duration.as_secs() < 10, "Plugin registration too slow");
        ...
        assert!(serialization_duration.as_secs() < 5, "AST serialization too slow");
    }
    ...
    async fn test_large_data_transfer() {
        ...
        assert!(!serialized.is_empty());
        assert!(serialization_time.as_secs() < 10, "Large AST serialization too slow: {:?}", serialization_time);
        ...
        assert!(deserialization_time.as_secs() < 5, "Large AST deserialization too slow: {:?}", deserialization_time);
    }
    ...
    async fn test_plugin_error_recovery() {
        ...
        let result = registry.register_plugin(valid_manifest.clone(), invalid_binary.to_vec()).await;
        ...
        let plugin_id = registry.register_plugin(valid_manifest, valid_binary).await.unwrap();
    ...
    async fn test_timeout_scenarios() {
        ...
        assert!(elapsed < Duration::from_secs(1), "Serialization should be fast");
    }
    ...
    fn create_large_test_ast() -> uveddi::ast::tree_sitter::ParsedFile {
        ...
        ParsedFile {
            ...
            ast: Some(AstNode {
                ...
                start_position: Position { row: 0, column: 0 },
                end_position: Position { row: 100, column: 0 },
                is_named: true,
                children,
            }),
        }
    ...
    fn create_very_large_test_ast() -> uveddi::ast::tree_sitter::ParsedFile {
        ...
        for i in 0..1000 {
            ...
            for j in 0..5 {
                nested_children.push(AstNode {
                    ...
                    start_position: Position { row: i, column: j * 10 },
                    end_position: Position { row: i, column: (j + 1) * 10 },
                    is_named: true,
                    children: vec![],
                });
            }
        ...
        ParsedFile {
            ...
            ast: Some(AstNode {
                ...
                start_position: Position { row: 0, column: 0 },
                end_position: Position { row: 1000, column: 0 },
                is_named: true,
                children,
            }),
        }
