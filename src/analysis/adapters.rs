...
impl AstParserTrait for AstParserAdapter {
    fn parse_file(&self, path: &Path) -> Result<ParsedFile, AnalysisError> {
        let mut parser = self.parser.lock().map_err(|e| {
            AnalysisError::DetectionError(format!("Parser lock failed: {}", e))
        })?;
        
        parser.parse_file(path).map_err(|e| {
            AnalysisError::DetectionError(format!("AST parsing failed for {}: {}", path.display(), e))
        })
    }
...
impl DependencyExtractorAdapter {
    ...
    pub fn new_default() -> Result<Self, AnalysisError> {
        let extractor = DependencyExtractor::new().map_err(|e| {
            AnalysisError::DetectionError(format!("Failed to initialize dependency extractor: {}", e))
        })?;
        Ok(Self { extractor })
    }
}
...
impl ResultCacheAdapter {
    ...
    pub fn new_with_path(path: &Path) -> Result<Self, AnalysisError> {
        let cache = ResultCache::new(path).map_err(|e| {
            AnalysisError::DetectionError(format!("Failed to initialize result cache at {}: {}", path.display(), e))
        })?;
        Ok(Self { 
            cache: Arc::new(Mutex::new(cache))
        })
...
impl ResultCacheTrait for ResultCacheAdapter {
    fn get_json(&self, key: &str) -> Result<Option<String>, AnalysisError> {
        let cache = self.cache.lock().map_err(|e| {
            AnalysisError::DetectionError(format!("Cache lock failed: {}", e))
        })?;
        ...
        match cache.get::<_, String>(&path_buf) {
            Ok(opt) => Ok(opt),
            Err(e) => Err(AnalysisError::DetectionError(format!("Cache get operation failed: {}", e)))
        }
    }
