...
impl DefaultsAnalyzer {
    ...
    fn build_common_patterns() -> Result<Vec<CommonDefaultPattern>, AnalysisError> {
        let patterns = vec![
            CommonDefaultPattern {
                ...
                regex: Regex::new(r#"(?i)(example|template|sample|demo|placeholder)[\s]*[:=][\s]*['"]?[^'"\n]*['"]?"#)?,
            ...
            CommonDefaultPattern {
                ...
                regex: Regex::new(r#"(?i)(todo|fixme|change[_-]?me|replace[_-]?me)[\s]*[:=][\s]*['"]?[^'"\n]*['"]?"#)?,
            ...
            CommonDefaultPattern {
                ...
                severity: ConfigSeverity::Medium,
                regex: Regex::new(r#"(?i)(host|server|url)[\s]*[:=][\s]*['"]?(localhost|127\.0\.0\.1)['"]?"#)?,
            ...
            CommonDefaultPattern {
                ...
                severity: ConfigSeverity::Medium,
                regex: Regex::new(r#"(?i)(test[_-]?mode|debug[_-]?mode|dev[_-]?mode)[\s]*[:=][\s]*['"]?(true|yes|1|on)['"]?"#)?,
    ...
    fn check_common_default_patterns(&self, content: &str) -> Result<Vec<ConfigIssue>, AnalysisError> {
        let mut issues = Vec::new();

        // Check for multiple default indicators in the same file
        let default_indicators = [
            "admin", "password", "secret", "test", "demo", "example",
            "localhost", "127.0.0.1", "changeme", "default"
        ];
        ...
        if found_indicators >= 5 {
            issues.push(ConfigIssue::new(
                ...
                format!("Configuration file contains {} default/example values", found_indicators),
            )
            .with_remediation("Review and replace all default values with production-appropriate settings.")
    ...
    fn check_development_indicators(&self, content: &str) -> Result<Vec<ConfigIssue>, AnalysisError> {
        let mut issues = Vec::new();
