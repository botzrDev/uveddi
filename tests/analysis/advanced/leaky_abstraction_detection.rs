...
mod tests {
    ...
    mod stub_tests {
        ...
        #[test]
        fn test_leaky_abstraction_detector_graceful_fallback() {
            ...
            assert!(
                result.is_ok(),
                "LeakyAbstractionDetector should not panic when tree-sitter is disabled"
            );
        }

        #[test]
        fn test_informative_skipping() {
            println!("SKIPPED: tree-sitter feature disabled - using fallback behavior for leaky abstraction detection");
