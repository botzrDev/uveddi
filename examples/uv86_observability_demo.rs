...
use uveddi::observability::{
    ObservabilityConfig, ObservabilityService, TraceId, generate_trace_id,
    InstrumentedCircuitBreaker, InstrumentedRetry, InstrumentedFallback,
};
...
async fn demo_metrics_collection(
    ...
    for i in 0..10 {
        ...
        metrics.record_request("POST", "/api/analyze", 
...
async fn demo_circuit_breaker(
    ...
    for i in 0..10 {
        ...
        let result = circuit_breaker.call(
            request_trace_id,
            simulate_external_service_call(i)
        ).await;
...
async fn demo_graceful_degradation(
    ...
    for i in 0..3 {
        ...
        let result = fallback.call_with_fallback(
            request_trace_id,
            "cache_fallback",
...
async fn simulate_external_service_call(request_id: usize) -> Result<String, UveddiError> {
    ...
    if request_id >= 2 && request_id <= 5 {
        ...
        Ok(format!("Response from external service for request {}", request_id))
    }
}
...
async fn simulate_flaky_operation(operation_id: usize) -> Result<String, UveddiError> {
    ...
    unsafe {
        ...
        if CALL_COUNT % 3 == 0 || CALL_COUNT % 3 == 2 {
            Ok(format!("Data retrieved successfully for operation {}", operation_id))
        } else {
