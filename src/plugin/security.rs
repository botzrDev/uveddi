        ...
        match permission {
            Permission::FilesystemRead { path } => {
                ...
                if is_safe_path(path) {
                    ...
                    return Err(UveddiError::PluginError(format!(
                        "Unsafe filesystem access requested: {}",
                        path
                    )));
                }
            }
            ...
            Permission::EnvironmentRead { var } => {
                ...
                if is_safe_env_var(var) {
                    ...
                    return Err(UveddiError::PluginError(format!(
                        "Access to sensitive environment variable denied: {}",
                        var
                    )));
                }
            }
...
fn is_safe_path(path: &str) -> bool {
    let safe_paths = ["/tmp/", "./plugins/", "./temp/", "./cache/"];

    // Don't allow access to system directories or parent directory traversal
    if path.contains("..")
        || path.starts_with('/') && !safe_paths.iter().any(|safe| path.starts_with(safe))
    {
        return false;
    }
...
fn is_safe_env_var(var: &str) -> bool {
    let safe_vars = ["UVEDDI_PLUGIN_CONFIG", "UVEDDI_DEBUG_LEVEL", "TEMP", "TMP"];
...
pub fn validate_resource_limits(limits: &ResourceLimits) -> Result<(), UveddiError> {
    ...
    if limits.max_execution_fuel > MAX_FUEL {
        return Err(UveddiError::PluginError(format!(
            "Execution fuel {} exceeds maximum allowed {}",
    ...
    if limits.max_output_size_kb > MAX_OUTPUT_KB {
        return Err(UveddiError::PluginError(format!(
            "Output size {} KB exceeds maximum allowed {} KB",
