    ...
    pub async fn verify_plugin(
        ...
        if policy.require_code_signing {
            report.signature_verification =
                Some(self.signature_verifier.verify(binary, manifest).await?);
        }
    ...
    fn audit_capabilities(
        ...
        for permission in &manifest.permissions {
            if policy.has_permission(permission) {
                ...
                audit
                    .warnings
                    .push(format!("Permission denied by policy: {:?}", permission));
            }
        }
        ...
        if audit.granted_permissions.len() > 10 {
            audit
                .warnings
                .push("Plugin requests an unusually high number of permissions".to_string());
        }
        ...
        for pattern in suspicious_strings {
                ...
                .position(|window| window == *pattern)
            {
                report.vulnerabilities.push(Vulnerability {
                    ...
                    description: format!(
                        "Suspicious string pattern found: {:?}",
...
impl ManifestValidator {
    ...
    pub fn validate(
        ...
        if manifest.name.is_empty() {
            validation
                .errors
                .push("Plugin name is required".to_string());
        ...
        if manifest.version.is_empty() {
            validation
                .errors
                .push("Plugin version is required".to_string());
            validation.is_valid = false;
        }
        ...
        if manifest.author.is_empty() {
            validation
                .errors
                .push("Plugin author is required".to_string());
            validation.is_valid = false;
        }
        ...
        if !self.is_valid_semver(&manifest.version) {
            validation
                .errors
                .push("Plugin version must be valid semver".to_string());
            validation.is_valid = false;
        }
        ...
        if manifest.supported_languages.is_empty() {
            validation
                .warnings
                .push("No supported languages specified".to_string());
        }
        ...
        if manifest.anti_pattern_types.is_empty() {
            validation
                .warnings
                .push("No anti-pattern types specified".to_string());
        }
        ...
        if policy.require_code_signing && manifest.signature.is_none() {
            validation
                .errors
                .push("Code signing required but no signature found".to_string());
            validation.is_valid = false;
        }
