...
use uveddi::monitoring::{
    ConfigManager, ReportingEngine, ReportScheduler,
    DistributionManager, ReportType, StakeholderRole,
    TestMetrics,
    metrics::{TestStatus, ResourceUsage},
    distribution::{DistributionConfigBuilder, SmtpConfig, SlackWebhookConfig},
    scheduler::{ScheduleBuilder, schedules},
    config::presets,
};
...
async fn example_config_loading() -> Result<()> {
    ...
    println!("  - System enabled: {}", config_manager.system_config().enabled);
    println!("  - Report jobs: {}", config_manager.report_jobs().len());
    println!("  - Distribution channels: {}", config_manager.distribution_config().channels.len());
...
async fn example_programmatic_setup() -> Result<()> {
    ...
    slack_webhooks.insert("main".to_string(), SlackWebhookConfig {
...
async fn example_manual_reports() -> Result<()> {
    ...
    let report_types = vec![
        ...
        (ReportType::PerformanceOptimization, StakeholderRole::Developer),
    ];
