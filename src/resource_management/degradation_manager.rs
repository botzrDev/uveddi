...
use super::{
    ...
    resource_config::{ResourceConfig, DegradationConfig},
...
impl GracefulDegradationManager {
    ...
    pub fn is_feature_enabled(&self, category: &FeatureCategory) -> bool {
        self.feature_states.lock().unwrap()
            .get(category)
            .copied()
            .unwrap_or(false)
    }
    ...
    pub async fn adjust_service_level(&self) -> ResourceResult<bool> {
        ...
        if new_level != current_level {
            self.apply_degradation_level(new_level.clone(), "Resource pressure adjustment".to_string()).await?;
            Ok(true)
        } else {
            // Check for recovery conditions
            self.check_recovery_conditions(memory_pressure, degradation_config).await?;
            Ok(false)
        }
    }
    
    /// Forces a specific degradation level (for emergency situations)
    pub async fn force_degradation_level(&self, level: DegradationLevel, reason: String) -> ResourceResult<()> {
    ...
    pub fn get_degradation_history(&self, duration: Duration) -> Vec<(Instant, DegradationLevel, String)> {
    ...
    pub async fn set_feature_enabled(&self, category: FeatureCategory, enabled: bool) -> ResourceResult<()> {
    ...
    fn determine_degradation_level(&self, memory_pressure: f64, config: &DegradationConfig) -> DegradationLevel {
    ...
    async fn check_recovery_conditions(&self, memory_pressure: f64, config: &DegradationConfig) -> ResourceResult<()> {
        let current_level = self.get_current_level();
            ...
            if recovery_timer.is_none() {
                *recovery_timer = Some(Instant::now());
                ...
                if timer.elapsed() > Duration::from_secs(30) {
                    ...
                    self.apply_degradation_level(recovery_level, "Automatic recovery".to_string()).await?;
                    *recovery_timer = None;
                }
            }
        } else {
    ...
    async fn apply_degradation_level(&self, level: DegradationLevel, reason: String) -> ResourceResult<()> {
