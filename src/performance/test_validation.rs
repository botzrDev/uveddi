...
mod tests {
    ...
    async fn test_phase2_integration() {
        println!("=== Phase 2: Criterion.rs Integration & Benchmark Correlation - Integration Test ===");
        ...
        println!("   - Mann-Kendall trend test: {:?}, p-value: {:.6}", mk_result.trend, mk_result.p_value);
        ...
        let mut baseline_manager = BenchmarkBaselineManager::new(
            temp_dir.path().join("baselines.json"),
            baseline_config,
        ).await.unwrap();
        ...
        let baseline = baseline_manager.create_baseline(
            "test_benchmark",
            baseline_measurements,
            baseline_type.clone(),
        ).await.unwrap();
        
        println!("   - Created baseline: {} samples, mean: {:.2}", 
        ...
        println!("   - Performance change: {:.1}%", 
                comparison.comparison_result.performance_change_percent);
        println!("   - Statistical confidence: {:.3}", comparison.statistical_confidence);
        ...
        let mut integration_manager = CriterionIntegrationManager::new(integration_config).await.unwrap();
        ...
        println!("   - Criterion result: mean {:.2} ns, {} samples", 
                criterion_result.mean_ns, criterion_result.sample_count);
        
        // Process benchmark result
        let integrated_result = integration_manager.process_benchmark_result(criterion_result).await.unwrap();
        
        println!("   - Regression verdict: {:?}", integrated_result.regression_verdict);
        println!("   - Statistical confidence: {:.3}", 
                integrated_result.statistical_analysis.confidence_interval_95.1 - 
                integrated_result.statistical_analysis.confidence_interval_95.0);
        ...
        if let Some(comparison) = integrated_result.baseline_comparison {
            ...
            println!("   - Generated report with {} benchmarks", 
            ...
            report_generator.export_html(&report, &html_path).await.unwrap();
            report_generator.export_json(&report, &json_path).await.unwrap();
