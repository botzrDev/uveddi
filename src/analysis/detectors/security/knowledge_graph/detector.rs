...
impl KnowledgeGraphDetector {
    ...
    async fn process_file(&self, file_path: &PathBuf) -> Result<FileAnalysis, AnalysisError> {
        ...
        let content = match std::fs::read_to_string(file_path) {
            ...
            Err(e) => {
                ...
                return Err(AnalysisError::FileReadError(e.to_string()));
            }
        };
    ...
    fn convert_patterns_to_issues(&self, patterns: &[crate::analysis::detectors::security::knowledge_graph::types::AntiPatternInfo]) -> Vec<SecurityIssue> {
        patterns.iter().map(|pattern| {
            SecurityIssue {
                ...
                line_number: None,
                code_snippet: None,
                recommendation: format!("Address the {} anti-pattern to improve security", pattern.pattern_type),
                cwe_id: None,
    ...
    fn map_pattern_to_issue_type(&self, pattern_type: &str) -> SecurityIssueType {
        match pattern_type.to_lowercase().as_str() {
            ...
            s if s.contains("trust") => SecurityIssueType::TrustBoundaryViolation,
            s if s.contains("auth") => SecurityIssueType::WeakAuthentication,
            s if s.contains("coupling") => SecurityIssueType::ArchitecturalWeakness,
            _ => SecurityIssueType::Other,
        }
    }
