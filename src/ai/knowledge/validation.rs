...
impl KnowledgeValidator {
    ...
    pub fn validate_library(&self, library: &KnowledgeLibrary) -> ValidationReport {
        ...
        if library.universal_patterns.is_empty() && library.language_specific.is_empty() {
            issues.push(ValidationIssue {
                ...
                suggestion: Some(
                    "Add universal anti-pattern definitions to the library".to_string(),
                ),
            });
        }
        ...
        let is_valid = issues
            .iter()
            .all(|i| i.severity != Severity::Critical && i.severity != Severity::Error);

        info!(
            "Validation completed: {} issues found, valid: {}",
            issues.len(),
            is_valid
        );
        ...
        for (pattern_id, pattern) in &library.universal_patterns {
            ...
            if pattern.definition.is_empty() {
                issues.push(ValidationIssue {
                    ...
                    suggestion: Some(
                        "Provide a comprehensive definition explaining what this anti-pattern is"
                            .to_string(),
                    ),
                });
            }
            ...
            if pattern.symptoms.len() < self.min_symptoms_per_pattern {
                ...
                issues.push(ValidationIssue {
                    ...
                    message: format!(
                        "Pattern has only {} symptoms, minimum is {}",
                        pattern.symptoms.len(),
                        self.min_symptoms_per_pattern
                    ),
                    suggestion: Some(
                        "Add more observable symptoms to help developers identify this pattern"
                            .to_string(),
                    ),
                });
            }
            ...
            if pattern.solutions.len() < self.min_solutions_per_pattern {
                ...
                issues.push(ValidationIssue {
                    ...
                    message: format!(
                        "Pattern has only {} solutions, minimum is {}",
                        pattern.solutions.len(),
                        self.min_solutions_per_pattern
                    ),
                    suggestion: Some(
                        "Provide at least one practical solution for addressing this anti-pattern"
                            .to_string(),
                    ),
                });
            }
            ...
            if pattern.examples.primary.len() < self.min_examples_per_pattern {
                ...
                issues.push(ValidationIssue {
                    ...
                    message: format!(
                        "Pattern has only {} examples, minimum is {}",
                        pattern.examples.primary.len(),
                        self.min_examples_per_pattern
                    ),
                    suggestion: Some(
                        "Add concrete code examples showing the anti-pattern and its solution"
                            .to_string(),
                    ),
                });
            }
            ...
            if pattern.detection_methods.len() < self.min_detection_methods_per_pattern {
                ...
                issues.push(ValidationIssue {
                    ...
                    message: format!(
                        "Pattern has only {} detection methods, minimum is {}",
                        pattern.detection_methods.len(),
                        self.min_detection_methods_per_pattern
                    ),
                    suggestion: Some(
                        "Define how automated tools can detect this anti-pattern".to_string(),
                    ),
                });
            }
            ...
            for (i, solution) in pattern.solutions.iter().enumerate() {
                if solution.implementation.is_empty() {
                    issues.push(ValidationIssue {
                        ...
                        suggestion: Some(
                            "Provide detailed implementation guidance for this solution"
                                .to_string(),
                        ),
                    });
                }
                ...
                if solution.examples.is_empty() && self.strict_mode {
                    issues.push(ValidationIssue {
                        ...
                        suggestion: Some(
                            "Add code examples demonstrating how to implement this solution"
                                .to_string(),
                        ),
                    });
                }
            ...
            for (pattern_id, pattern) in &lang_knowledge.patterns {
                ...
                if pattern.definition.is_empty() {
                    issues.push(ValidationIssue {
                        ...
                        suggestion: Some(
                            "Provide a definition specific to this programming language"
                                .to_string(),
                        ),
                    });
                }
        ...
        for (pattern_id, pattern) in &library.universal_patterns {
            ...
            if pattern.definition.len() < 50 {
                issues.push(ValidationIssue {
                    ...
                    suggestion: Some(
                        "Provide more context and explanation for better understanding".to_string(),
                    ),
                });
            }
                ...
                for term in &vague_terms {
                    if symptom_text.contains(term) && !symptom_text.contains("(") {
                        issues.push(ValidationIssue {
                            ...
                            field: Some(format!("symptoms[{}]", i)),
                            message: format!(
                                "Consider making '{}' more specific with thresholds or examples",
                                term
                            ),
                            suggestion: Some(
            ...
            if pattern.frequency_score < 0.0 || pattern.frequency_score > 1.0 {
                issues.push(ValidationIssue {
                    ...
                    suggestion: Some(
                        "Set frequency score based on how common this pattern is in practice"
                            .to_string(),
                    ),
                });
            }
            ...
            if pattern.detection_confidence < 0.0 || pattern.detection_confidence > 1.0 {
                issues.push(ValidationIssue {
                    ...
                    suggestion: Some(
                        "Set confidence based on how reliably this pattern can be detected"
                            .to_string(),
                    ),
                });
            }
    ...
    fn validate_relationships(
        ...
        let all_pattern_ids: HashSet<String> = library
            .universal_patterns
            .keys()
            .chain(
                library
        ...
        for (pattern_id, pattern) in &library.universal_patterns {
            ...
            for related_id in &pattern.related_patterns {
                if !all_pattern_ids.contains(related_id) {
                    issues.push(ValidationIssue {
                        ...
                        suggestion: Some(
                            "Remove invalid reference or create the referenced pattern".to_string(),
                        ),
                    });
                }
            }
            ...
            if !pattern.id.chars().all(|c| c.is_lowercase() || c == '_') {
                issues.push(ValidationIssue {
                    ...
                    suggestion: Some(
                        "Use lowercase letters and underscores for pattern IDs".to_string(),
                    ),
                });
            }
            ...
            if !pattern
                .tags
                .iter()
                .any(|tag| category_str.contains(&tag.to_lowercase()))
            {
                issues.push(ValidationIssue {
                    severity: Severity::Info,
                    pattern_id: Some(pattern_id.clone()),
    ...
    fn validate_compression_readiness(
        &self,
        library: &KnowledgeLibrary,
        issues: &mut Vec<ValidationIssue>,
    ) {
        ...
        for (pattern_id, pattern) in &library.universal_patterns {
            ...
            if pattern.definition.len() > 2000 {
                issues.push(ValidationIssue {
                    ...
                    suggestion: Some(
                        "Consider breaking down into smaller, more focused sections".to_string(),
                    ),
                });
            }

            // Check for compression-unfriendly characters
            let problematic_chars = pattern
                .definition
                .as_str()
                .chars()
            ...
            if problematic_chars > pattern.definition.len() / 10 {
                issues.push(ValidationIssue {
                    ...
                    pattern_id: Some(pattern_id.clone()),
                    field: Some("definition".to_string()),
                    message: "High ratio of non-ASCII characters may reduce compression efficiency"
                        .to_string(),
                    suggestion: Some(
                        "Consider using ASCII alternatives where possible".to_string(),
                    ),
                });
            }
        }
        ...
        for (phrase, count) in repeated_phrases {
            if count >= 3 && phrase.len() > 10 {
                issues.push(ValidationIssue {
                    ...
                    message: format!(
                        "Phrase '{}' repeated {} times - good dictionary candidate",
                        phrase, count
                    ),
                    suggestion: Some(
                        "This phrase should be included in the compression dictionary".to_string(),
                    ),
                });
            }
        }
                    ...
                    DetectionMethod::MetricThreshold { threshold, .. } => {
                        if *threshold <= 0.0 {
                            issues.push(ValidationIssue {
                                ...
                                suggestion: Some(
                                    "Set a meaningful positive threshold value".to_string(),
                                ),
                            });
                        }
                    }
                    DetectionMethod::StaticAnalysis { confidence, .. } => {
                        if *confidence < 0.0 || *confidence > 1.0 {
                            issues.push(ValidationIssue {
                                ...
                                pattern_id: Some(pattern_id.clone()),
                                field: Some(format!("detection_methods[{}]", i)),
                                message: "Static analysis confidence must be between 0.0 and 1.0"
                                    .to_string(),
                                suggestion: Some(
                                    "Set confidence based on method reliability".to_string(),
                                ),
                            });
                        }
                    }
                    DetectionMethod::AstPattern { node_types, .. } => {
                        if node_types.is_empty() {
                            issues.push(ValidationIssue {
                                ...
                                suggestion: Some(
                                    "Specify the AST node types this pattern matches".to_string(),
                                ),
                            });
                        }
                    }
    ...
    fn generate_summary(
        ...
        let total_solutions: usize = library
            .universal_patterns
            .values()
    ...
    fn generate_recommendations(
        ...
        let incomplete_patterns = library
            .universal_patterns
            .values()
        ...
        let patterns_without_examples = library
            .universal_patterns
            .values()
        ...
        recommendations.push(
            "Run compression analysis to optimize dictionary training for zstd compression"
                .to_string(),
        );
        ...
        if library.universal_patterns.len() < 15 {
            recommendations.push(
                "Consider adding more anti-pattern definitions to provide comprehensive coverage"
                    .to_string(),
            );
        }
...
mod tests {
    ...
    fn test_validate_populated_library() {
        ...
        println!(
            "Validation report: {} issues, coverage: {:.2}, quality: {:.2}",
            report.issues.len(),
