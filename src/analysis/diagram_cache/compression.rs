...
impl CompressionEngine {
    ...
    pub async fn compress(&self, content: &[u8], diagram_type: &DiagramType) -> Result<Vec<u8>> {
        ...
        debug!(
            "Compressed {:?} diagram: {} -> {} bytes ({:.1}% reduction, {}ms)",
            diagram_type,
            content.len(),
            compressed.len(),
...
mod tests {
    ...
    async fn test_text_compression() {
        ...
        let compressed = engine
            .compress(content, &DiagramType::Mermaid)
            .await
            .unwrap();
        let decompressed = engine
            .decompress(&compressed, &DiagramType::Mermaid)
            .await
            .unwrap();
    ...
    fn test_compression_ratio_estimation() {
        ...
        let repetitive_ratio =
            engine.estimate_compression_ratio(&repetitive_content, &DiagramType::Mermaid);
