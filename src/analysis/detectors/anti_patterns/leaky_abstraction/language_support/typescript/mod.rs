...
impl TypeScriptLanguageSupport {
    ...
    pub fn analyze_file(
        ...
        issues.extend(self.interface_parser.analyze_interface_violations(parsed_file, context)?);
        issues.extend(self.interface_parser.analyze_type_exposure(parsed_file, context)?);

        // Analyze class structure
        issues.extend(self.class_analyzer.analyze_dom_coupling(parsed_file, context)?);
        issues.extend(self.class_analyzer.analyze_class_inheritance(parsed_file, context)?);
        issues.extend(self.class_analyzer.analyze_class_properties(parsed_file, context)?);

        // Analyze module boundaries
        issues.extend(self.module_analyzer.analyze_import_violations(parsed_file, context)?);
        issues.extend(self.module_analyzer.analyze_export_patterns(parsed_file, context)?);
