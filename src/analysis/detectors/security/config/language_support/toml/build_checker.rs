...
impl TomlBuildChecker {
    ...
    pub fn check_build_configuration(
        &self,
        value: &TomlValue,
                        ...
                        if self.is_external_workspace_member(member_path) {
                            issues.push(
                                utils::create_config_issue(
                                    ...
                                    format!(
                                        "Workspace member points to external path: {}",
                                        member_path
                                    ),
                                    None,
                    ...
                    if let TomlValue::String(exclude_path) = excluded {
                        if exclude_path.contains("security") || exclude_path.contains("audit") {
                            issues.push(utils::create_config_issue(
                                ...
                                format!(
                                    "Workspace excludes security-related directory: {}",
                                    exclude_path
                                ),
                                None,
...
"#;
        ...
        assert!(issues
            .iter()
            .any(|i| i.title.contains("Unsafe Feature Flag")));
    }
...
"#;
        ...
        assert!(issues
            .iter()
            .any(|i| i.title.contains("Build Script Present")));
    }
...
"#;
        ...
        assert!(issues
            .iter()
            .any(|i| i.title.contains("Debug Info in Release")));
        assert!(issues
            .iter()
            .any(|i| i.title.contains("Overflow Checks Disabled")));
    }
...
"#;
        ...
        assert!(issues
            .iter()
            .any(|i| i.title.contains("External Workspace Member")));
        assert!(issues
            .iter()
            .any(|i| i.title.contains("Security Directory Excluded")));
    }
