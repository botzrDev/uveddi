...
use super::utils::{
    calculate_severity_distribution, extract_class_name, generate_recommendations,
    get_severity_icon,
};
...
impl MarkdownFormatter {
    ...
    pub fn format(issues: &[ArchitecturalIssue]) -> String {
        ...
        if issues.is_empty() {
            markdown.push_str(
                "✅ **No God Objects detected!** Your code follows good separation of concerns.\n",
            );
            return markdown;
        }
    ...
    fn format_issue(issue: &ArchitecturalIssue, severity: &str, index: usize) -> String {
        ...
        markdown.push_str(&format!(
            ...
            severity,
            index,
            extract_class_name(&issue.description)
        ));
    ...
    fn truncate_snippet(snippet: &str) -> String {
        if snippet.len() > 1000 {
            format!(
                "{}...\n\n[Code snippet truncated - {} characters total]",
                &snippet[..1000],
                snippet.len()
            )
        } else {
    ...
    fn get_general_recommendations() -> String {
        ...
        markdown
            .push_str("2. **Extract Classes**: Break large classes into smaller, focused ones\n");
        markdown.push_str(
            "3. **Composition over Inheritance**: Use composition to combine behaviors\n",
        );
        markdown.push_str(
            "4. **Interface Segregation**: Create focused interfaces instead of large ones\n",
        );
        ...
        markdown.push_str(
            "- **Replace Data Value with Object**: Create value objects for complex data\n",
        );
        markdown.push_str(
            "- **Strategy Pattern**: Extract varying behaviors into separate strategies\n",
        );
    ...
    pub fn generate_summary(all_issues: &[ArchitecturalIssue]) -> String {
        ...
        if all_issues.is_empty() {
            summary.push_str(
                "✅ **Excellent!** No God Objects detected across the analyzed codebase.\n",
            );
            return summary;
        }
        ...
        summary.push_str("\n## Next Steps\n\n");
        summary
            .push_str("1. 🎯 **Start with Critical issues** - These require immediate attention\n");
        summary.push_str("2. 📋 **Prioritize by file** - Focus on files with multiple issues\n");
        summary.push_str(
            "3. 🔄 **Refactor incrementally** - Break down one responsibility at a time\n",
        );
