...
impl MigrationManager {
    ...
    pub async fn migrate_to_version(&self, target_version: u32) -> Result<MigrationResult> {
        ...
        let migrations = self
            .load_migrations(current_version + 1, target_version)
            .await?;
        ...
        for migration in migrations {
            ...
            match self.apply_migration(&migration).await {
                Ok(()) => {
                    ...
                    info!(
                        "Successfully applied migration {} in {:?}",
    ...
    async fn load_migrations(&self, from_version: u32, to_version: u32) -> Result<Vec<Migration>> {
        ...
        let migration_files = std::fs::read_dir(&self.migrations_dir).map_err(|e| {
            UveddiError::io_error("read_dir", &self.migrations_dir.to_string_lossy(), e)
        })?;

        for entry in migration_files {
            let entry = entry.map_err(|e| {
                UveddiError::io_error("read_dir_entry", &self.migrations_dir.to_string_lossy(), e)
            })?;
            let path = entry.path();
    ...
    async fn parse_migration_file(&self, path: &Path) -> Result<Option<Migration>> {
        ...
        let version = version_str.parse::<u32>().map_err(|_| {
            UveddiError::validation_error(&format!("Invalid migration version: {}", version_str))
        })?;
    ...
    async fn apply_migration(&self, migration: &Migration) -> Result<()> {
        ...
        for statement in statements {
            ...
            tx.execute_write(statement, &[]).await.map_err(|e| {
    ...
    pub async fn migrate_sqlite_to_postgresql(
        ...
        for table_name in tables {
            ...
            match self
                .migrate_table_data(&*sqlite_provider, &*pg_provider, table_name)
                .await
            {
                Ok(row_count) => {
                    ...
                    info!(
                        "Successfully migrated {} rows from table {}",
    ...
    async fn migrate_table_data(
        ...
        let source_data = source_provider
    ...
    pub async fn create_migration(&self, name: &str) -> Result<PathBuf> {
        ...
        tokio::fs::create_dir_all(&self.migrations_dir)
            .await
            .map_err(|e| {
                UveddiError::io_error("create_dir_all", &self.migrations_dir.to_string_lossy(), e)
            })?;
