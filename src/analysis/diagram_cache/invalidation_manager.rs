...
use super::{Result, DiagramCacheError};
...
impl InvalidationManager {
    ...
    pub async fn should_invalidate_diagram(
        ...
        match &self.strategy {
            InvalidationStrategy::DirectDependency => {
                self.direct_dependency_decision(diagram_id, changeset, diagram_dependencies).await
            }
            InvalidationStrategy::DependencyBased => {
                self.dependency_based_decision(diagram_id, changeset, diagram_dependencies).await
            }
            InvalidationStrategy::TimeBased { ttl_hours } => {
                self.time_based_decision(diagram_id, *ttl_hours).await
            }
            InvalidationStrategy::Adaptive => {
                self.adaptive_decision(diagram_id, changeset, diagram_dependencies).await
            }
            InvalidationStrategy::Conservative => {
                self.conservative_decision(diagram_id, changeset, diagram_dependencies).await
            }
            InvalidationStrategy::Aggressive => {
                self.aggressive_decision(diagram_id, changeset, diagram_dependencies).await
            }
        }
    }
    ...
    pub fn record_regeneration_cost(&mut self, diagram_id: &str, cost_ms: u64) {
        self.regeneration_costs.insert(diagram_id.to_string(), cost_ms);
    }
    ...
    pub fn get_invalidation_stats(&self) -> InvalidationStats {
        ...
        let correct_invalidations = self.invalidation_history
            .iter()
        ...
        let avg_regeneration_time = if !self.regeneration_costs.is_empty() {
            self.regeneration_costs.values().sum::<u64>() as f64 / self.regeneration_costs.len() as f64
        } else {
            0.0
        };
        ...
        let most_changed_files = {
            ...
            files.into_iter().take(10).map(|(f, c)| (f.clone(), *c)).collect()
        };
    ...
    pub async fn optimize_strategy(&mut self) -> Result<()> {
        ...
        if stats.accuracy < 0.7 {
            warn!("Low invalidation accuracy ({:.1}%), switching to conservative strategy", stats.accuracy * 100.0);
        ...
        // If accuracy is very high, we might be too conservative
            ...
            info!("High invalidation accuracy ({:.1}%), switching to aggressive strategy", stats.accuracy * 100.0);
    ...
    async fn dependency_based_decision(
        ...
        let direct_change = self.direct_dependency_decision(diagram_id, changeset, diagram_dependencies).await?;
        ...
        let has_indirect_change = diagram_dependencies.iter().any(|dep| {
            changeset.affected_by_dependencies.contains(dep)
        });
    ...
    async fn time_based_decision(&self, diagram_id: &str, ttl_hours: u32) -> Result<InvalidationDecision> {
    ...
    async fn adaptive_decision(
        ...
        let base_decision = self.dependency_based_decision(diagram_id, changeset, diagram_dependencies).await?;
        
        // Adjust based on historical patterns
        let change_frequency = diagram_dependencies.iter()
        ...
        let adjusted_confidence = base_decision.confidence * (0.5 + 0.5 * frequency_factor * cost_factor);
        
        Ok(InvalidationDecision {
            should_invalidate: adjusted_confidence > 0.5,
            confidence: adjusted_confidence,
            reasoning: format!("Adaptive: base={:.2}, freq={}, cost={}ms", 
    ...
    async fn aggressive_decision(
        ...
        Ok(InvalidationDecision {
            ...
            priority: if base_decision.priority > 0 { base_decision.priority - 1 } else { 0 },
        })
    }
}
