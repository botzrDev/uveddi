...
fn test_multilang_ast_parsing_and_caching() {
    ...
    File::create(&rust_path).unwrap().write_all(b"struct Foo { bar: i32 }\n").unwrap();
    File::create(&py_path).unwrap().write_all(b"class Foo:\n    pass\n").unwrap();
    File::create(&js_path).unwrap().write_all(b"class Foo {}\n").unwrap();
