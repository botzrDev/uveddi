...
use uveddi::monitoring::{
    distribution::{email_channel, slack_channel, DistributionConfigBuilder},
    metrics::{ResourceUsage, TestStatus},
    scheduler::{schedules, RetryConfig, ScheduleBuilder},
    DistributionChannel, DistributionManager, ReportConfiguration, ReportScheduler, ReportType,
    ReportingEngine, ScheduleConfig, ScheduleType, SlackWebhookConfig, SmtpConfig, StakeholderRole,
    TestMetrics,
};
...
fn create_basic_config(report_type: ReportType, role: StakeholderRole) -> ReportConfiguration {
    use uveddi::monitoring::reporting::{BrandingConfig, ScheduleConfig, TemplateCustomization};
...
async fn test_stakeholder_customization() -> Result<()> {
    ...
    for role in stakeholders {
        ...
        if !insights.is_empty() {
            let relevant_insights = insights
                .iter()
                .filter(|insight| insight.stakeholder_relevance.contains(&role))
                .count();
            assert!(
                relevant_insights > 0,
                "No relevant insights for role {:?}",
                role
            );
        }
        ...
        if !recommendations.is_empty() {
            let role_recommendations = recommendations
                .iter()
                .filter(|rec| {
                    rec.assigned_role.as_ref() == Some(&role) || rec.assigned_role.is_none()
                })
                .count();
            assert!(
                role_recommendations > 0,
                "No relevant recommendations for role {:?}",
                role
            );
        }
    }
...
async fn test_report_scheduler_basic() -> Result<()> {
    ...
    let schedule = ScheduleBuilder::new().daily_at("09:00").build()?;

    let config = create_basic_config(ReportType::DailyHealth, StakeholderRole::QaEngineer);

    let job_id = scheduler.add_job("Daily Health Report".to_string(), config, schedule)?;
...
async fn test_report_store_search() -> Result<()> {
    ...
    let daily_reports = search_results
        .iter()
...
async fn test_executive_summary_metrics() -> Result<()> {
    ...
    assert!(report.content.insights.iter().any(|insight| insight
        .stakeholder_relevance
        .contains(&StakeholderRole::Executive)
        || insight
            .stakeholder_relevance
            .contains(&StakeholderRole::Manager)));
...
async fn test_concurrent_report_generation() -> Result<()> {
    ...
    for config in configs {
        ...
        let handle = tokio::spawn(async move { engine_clone.generate_report(&config).await });
        handles.push(handle);
    }
...
async fn test_integration_full_workflow() -> Result<()> {
    ...
    let job_id = scheduler.add_job("Weekly QA Report".to_string(), config.clone(), schedule)?;
...
async fn test_large_dataset_performance() -> Result<()> {
    ...
    assert!(
        render_time.as_secs() < 5,
        "Report rendering took too long: {:?}",
        render_time
    );
    assert!(!rendered.is_empty());
