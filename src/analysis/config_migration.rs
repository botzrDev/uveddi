...
use crate::analysis::standardized_config::{StandardDetectorConfig, StandardConfigBuilder, ConfigValue};
use crate::analysis::detectors::anti_patterns::{
    god_object::GodObjectConfig,
    code_duplication::DuplicationConfig,
...
impl ConfigMigration<GodObjectConfig> for GodObjectConfig {
    ...
    fn from_standard(config: &StandardDetectorConfig) -> Result<GodObjectConfig, UveddiError> {
        ...
        Ok(GodObjectConfig {
            method_thresholds,
            field_thresholds,
            framework_modules: config.exclusions.framework_modules.iter().cloned().collect(),
...
impl ConfigMigration<DuplicationConfig> for DuplicationConfig {
    ...
    fn from_standard(config: &StandardDetectorConfig) -> Result<DuplicationConfig, UveddiError> {
        Ok(DuplicationConfig {
            ...
            similarity_threshold: config.get_threshold_float("similarity_threshold", SourceLanguage::Rust)?,
            fingerprint_length: config.get_threshold_int("fingerprint_length", SourceLanguage::Rust)? as usize,
            ignore_identifiers: config.thresholds.get("ignore_identifiers")
                .map(|v| v.as_bool().unwrap_or(false))
                .unwrap_or(false),
            ignore_literals: config.thresholds.get("ignore_literals")
                .map(|v| v.as_bool().unwrap_or(false))
                .unwrap_or(false),
            ..Default::default()
        })
...
impl ConfigMigration<LargeClassConfig> for LargeClassConfig {
    fn to_standard(&self) -> Result<StandardDetectorConfig, UveddiError> {
        ...
        builder = builder
            .language_threshold("rust", "max_lines", self.rust_thresholds.max_logical_loc as i64)
            .language_threshold("rust", "max_methods", self.rust_thresholds.max_methods as i64)
            .language_threshold("rust", "max_fields", self.rust_thresholds.max_fields as i64)
            .language_threshold("rust", "max_complexity", self.rust_thresholds.max_cyclomatic_complexity as i64);
        
        // Convert Python thresholds
        builder = builder
            .language_threshold("python", "max_lines", self.python_thresholds.max_logical_loc as i64)
            .language_threshold("python", "max_methods", self.python_thresholds.max_methods as i64)
            .language_threshold("python", "max_fields", self.python_thresholds.max_fields as i64)
            .language_threshold("python", "max_complexity", self.python_thresholds.max_cyclomatic_complexity as i64);
        
        // Convert JavaScript thresholds
        builder = builder
            .language_threshold("javascript", "max_lines", self.javascript_thresholds.max_logical_loc as i64)
            .language_threshold("javascript", "max_methods", self.javascript_thresholds.max_methods as i64)
            .language_threshold("javascript", "max_fields", self.javascript_thresholds.max_fields as i64)
            .language_threshold("javascript", "max_complexity", self.javascript_thresholds.max_cyclomatic_complexity as i64);
    ...
    fn from_standard(config: &StandardDetectorConfig) -> Result<LargeClassConfig, UveddiError> {
        use crate::analysis::detectors::anti_patterns::large_classes::{LanguageThresholds, SeverityWeights};
        ...
        let rust_thresholds = LanguageThresholds {
            ...
            max_cyclomatic_complexity: config.get_threshold_int("max_complexity", SourceLanguage::Rust)? as u32,
        ...
        let python_thresholds = LanguageThresholds {
            ...
            max_cyclomatic_complexity: config.get_threshold_int("max_complexity", SourceLanguage::Python)? as u32,
            max_cognitive_complexity: 50, // Default
            max_lcom_score: 0.8,         // Default
            max_coupling: 10,            // Default
        };
        ...
        let javascript_thresholds = LanguageThresholds {
            max_logical_loc: config.get_threshold_int("max_lines", SourceLanguage::JavaScript)? as u32,
            max_methods: config.get_threshold_int("max_methods", SourceLanguage::JavaScript)? as u32,
            max_fields: config.get_threshold_int("max_fields", SourceLanguage::JavaScript)? as u32,
            max_cyclomatic_complexity: config.get_threshold_int("max_complexity", SourceLanguage::JavaScript)? as u32,
            max_cognitive_complexity: 50, // Default
            max_lcom_score: 0.8,         // Default
            max_coupling: 10,            // Default
        };
...
mod tests {
    ...
    fn test_duplication_config_migration() {
        ...
        assert_eq!(converted_back.similarity_threshold, old_config.similarity_threshold);
    }
    
    #[test]
    fn test_toml_serialization() {
        let config = StandardDetectorConfig::default_for_detector("god_object");
        let toml_str = ConfigMigrationUtils::to_toml(&config)
