...
use crate::report::interactive_models::{
    Finding, InteractiveReport, ProjectMetadata, AnalysisSummary,
    DependencyGraph, DiagramDefinition, ReportMetadata, REPORT_SCHEMA_VERSION,
...
impl DataTransformer {
    ...
    pub fn transform_to_interactive_report(
        ...
        let metadata = ReportMetadata {
            ...
            performance: Some(crate::report::interactive_models::GenerationPerformance {
                analysis_duration_ms: analysis_run.end_time
                    .map(|end| (end - analysis_run.start_time).num_milliseconds() as u64)
                    .unwrap_or(0),
                generation_duration_ms: 0, // TODO: Track generation time
                peak_memory_bytes: None,
                files_per_second: analysis_run.total_files_analyzed
                    .and_then(|files| {
                        analysis_run.end_time
                            .map(|end| {
                                let duration_secs = (end - analysis_run.start_time).num_seconds() as f64;
    ...
    fn generate_title_from_issue(issue: &ArchitecturalIssue, anti_pattern: Option<&AntiPatternType>) -> String {
    ...
    fn generate_tags(issue: &ArchitecturalIssue, anti_pattern: Option<&AntiPatternType>) -> Vec<String> {
        let mut tags = vec![];
    ...
    fn generate_recommendation(issue: &ArchitecturalIssue, anti_pattern: Option<&AntiPatternType>) -> Option<String> {
        if let Some(apt) = anti_pattern {
