...
use uveddi::database::{
    providers::{DatabaseConfig, DatabaseType, create_database_provider},
...
async fn test_concurrent_load() -> Result<()> {
    ...
    database.store_anti_pattern_types_batch(&mut anti_patterns).await?;
    ...
    for i in 0..concurrent_operations {
        let db = database.clone();
        let anti_pattern_id = anti_patterns[i % anti_patterns.len()].anti_pattern_type_id.unwrap();
    ...
    assert!(ops_per_second > 10.0, "Performance below threshold: {} ops/sec", ops_per_second);
    assert!(failures < concurrent_operations / 10, "Too many failures: {}", failures);
...
async fn test_configuration_management() -> Result<()> {
    ...
    assert!(!masked.contains("password"), "Connection string should be masked");
...
async fn test_migration_system() -> Result<()> {
    ...
    tokio::fs::write(&migration_path, migration_sql).await
    ...
    assert!(!result.applied_migrations.is_empty(), "Should have applied migrations");
...
async fn benchmark_database_performance() -> Result<()> {
    ...
    let benchmark_results = vec![
        ...
        benchmark_operation("Read Operations", 200, || {
            let db = database.clone();
            async move {
                db.get_recent_analysis_runs(10).await.map(|_| ())
            }
        }).await,
    ];
    ...
    for (name, ops_per_sec, avg_duration) in benchmark_results {
        println!("{}: {:.2} ops/sec, avg: {:?}", name, ops_per_sec, avg_duration);
        
        // Performance assertions
        match name.as_str() {
            "Create Analysis Runs" => assert!(ops_per_sec > 50.0, "Create operations too slow: {:.2} ops/sec", ops_per_sec),
            "Read Operations" => assert!(ops_per_sec > 100.0, "Read operations too slow: {:.2} ops/sec", ops_per_sec),
            _ => {}
        }
    }
...
fn generate_test_issues(run_id: i64, anti_pattern_id: i64, count: usize) -> Vec<ArchitecturalIssue> {
    (0..count).map(|i| ArchitecturalIssue {
        issue_id: None,
        analysis_run_id: run_id,
        anti_pattern_type_id: anti_pattern_id,
        file_path: format!("/test/file_{}.rs", i),
