...
impl EnvValidator {
    ...
    pub fn check_common_issues(&self, key: &str, value: &str, line_number: usize) -> Vec<ConfigIssue> {
        ...
        if self.is_production_key(key) && self.contains_localhost(value) {
            issues.push(utils::create_config_issue(
                ...
                format!("Production environment variable '{}' contains localhost", key),
                Some(line_number),
    ...
    fn is_default_value(&self, value: &str) -> bool {
        let default_values = [
            "default", "example", "changeme", "replace-me", "your-value-here",
            "todo", "fixme", "placeholder", "sample", "demo", "test",
            "localhost", "127.0.0.1", "http://localhost", "http://example.com"
        ];

        let value_lower = value.to_lowercase();
        default_values.iter().any(|&default| value_lower.contains(default))
    }

    fn is_sensitive_key(&self, key: &str) -> bool {
        let sensitive_indicators = [
            "password", "secret", "key", "token", "auth", "credential",
            "private", "secure", "salt", "hash"
        ];

        let key_lower = key.to_lowercase();
        sensitive_indicators.iter().any(|&indicator| key_lower.contains(indicator))
    }
    ...
    fn is_production_key(&self, key: &str) -> bool {
        ...
        let key_lower = key.to_lowercase();
        prod_indicators.iter().any(|&indicator| key_lower.contains(indicator))
    }
    ...
    fn is_debug_key(&self, key: &str) -> bool {
        ...
        let key_lower = key.to_lowercase();
        debug_indicators.iter().any(|&indicator| key_lower.contains(indicator))
    }
    ...
    fn check_missing_required_vars(&self, content: &str) -> Vec<ConfigIssue> {
        ...
        let has_web_indicators = var_names.iter().any(|name| {
            ...
            name_lower.contains("port") || name_lower.contains("host") ||
            name_lower.contains("url") || name_lower.contains("database")
        });
            ...
            for (required_var, description) in &common_required {
                if !var_names.contains(*required_var) {
                    issues.push(utils::create_config_issue(
                        ...
                        format!("Missing common variable '{}' for {}", required_var, description),
                        None,
                        format!("Consider adding {} if needed for your application", required_var).to_string(),
    ...
    fn check_naming_conventions(&self, content: &str) -> Vec<ConfigIssue> {
        ...
        if naming_styles.len() > 1 {
            ...
            if total_vars > 5 { // Only suggest for larger files
                issues.push(utils::create_config_issue(
                    ConfigSeverity::Info,
                    "Inconsistent Naming Convention".to_string(),
                    "Environment variables use inconsistent naming styles".to_string(),
                    None,
                    "Consider using consistent UPPER_SNAKE_CASE for all environment variables".to_string(),
...
mod tests {
    ...
    fn test_default_value_detection() {
        ...
        assert!(issues.iter().any(|i| i.title.contains("Default Environment Value")));
    }
    ...
    fn test_empty_sensitive_value() {
        ...
        assert!(issues.iter().any(|i| i.title.contains("Empty Sensitive Value")));
    }
    ...
    fn test_localhost_in_production() {
        ...
        assert!(issues.iter().any(|i| i.title.contains("Localhost in Production")));
    }
    ...
    fn test_debug_mode_detection() {
        ...
        assert!(issues.iter().any(|i| i.title.contains("Debug Mode Enabled")));
    }
...
"#;
        ...
        assert!(issues.iter().any(|i| i.title.contains("Duplicate Environment Variable")));
    }
}