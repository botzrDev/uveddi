...
impl PluginCommand {
    ...
    pub async fn execute(self) -> Result<(), crate::error::UveddiError> {
        match self.action {
            ...
            PluginAction::Install { ref binary, ref manifest } => self.install_plugin(binary.clone(), manifest.clone()).await,
            ...
            PluginAction::Verify { ref binary, ref manifest } => self.verify_plugin(binary.clone(), manifest.clone()).await,
        }
    }
    ...
    async fn list_plugins(&self) -> Result<(), crate::error::UveddiError> {
        #[cfg(feature = "wasm-plugins")]
        {
            ...
            println!("  Supported anti-patterns: {}", stats.supported_anti_patterns.join(", "));
        ...
        {
            println!("WASM plugins not enabled. Compile with --features wasm-plugins to use plugins.");
        }
        
        Ok(())
    }

    async fn install_plugin(&self, binary_path: PathBuf, manifest_path: PathBuf) -> Result<(), crate::error::UveddiError> {
        #[cfg(feature = "wasm-plugins")]
        {
            println!("Installing plugin from {} and {}...", binary_path.display(), manifest_path.display());
            ...
            println!("  Supported languages: {}", manifest.supported_languages.join(", "));
            println!("  Anti-pattern types: {}", manifest.anti_pattern_types.join(", "));
        }
    ...
    async fn monitor_plugins(&self) -> Result<(), crate::error::UveddiError> {
        #[cfg(feature = "wasm-plugins")]
        {
            ...
            println!("  Total memory usage: {} bytes", resource_report.total_memory_usage);
            println!("  Total fuel consumed: {}", resource_report.total_fuel_consumed);
    ...
    async fn verify_plugin(&self, binary_path: PathBuf, manifest_path: PathBuf) -> Result<(), crate::error::UveddiError> {
        #[cfg(feature = "wasm-plugins")]
        {
            use crate::plugins::{PluginVerifier, SecurityPolicy};
            
            println!("Verifying plugin from {} and {}...", binary_path.display(), manifest_path.display());
            ...
            match verifier.verify_plugin(&binary, &manifest, &policy).await {
                Ok(report) => {
                    ...
                    println!("  Plugin: {} v{}", report.plugin_name, report.plugin_version);
                    println!("  Status: {:?}", report.overall_status);
                    println!("  Binary hash: {}", report.plugin_hash);
                    println!("  Verification timestamp: {}", report.verification_timestamp);
                    
                    println!("\nStatic Analysis:");
                    println!("  Vulnerabilities found: {}", report.static_analysis.vulnerabilities.len());
                    ...
                    println!("\nCapability Audit:");
                    println!("  Requested permissions: {}", report.capability_audit.requested_permissions.len());
                    println!("  Granted permissions: {}", report.capability_audit.granted_permissions.len());
                    println!("  Denied permissions: {}", report.capability_audit.denied_permissions.len());
                }
                Err(e) => {
