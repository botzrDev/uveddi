...
use uveddi::core::interfaces::{
    persistence::{
        DomainIssue, IssueSeverity, AnalysisRunDomain, MockPersistenceProvider, PersistenceProvider,
    },
    events::{EventBus, DomainEvent, AstEvent, AnalysisEvent, AstData, AstMetadata},
};
use std::sync::Arc;
use chrono::Utc;
...
async fn demonstrate_persistence_interface() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Saving {} issues via persistence interface...", issues.len());
...
async fn demonstrate_event_system() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let analysis_event = AnalysisEvent::analysis_completed(
        1,
        2,
        10,
...
struct ExampleAnalysisService {
    persistence: Arc<dyn PersistenceProvider<Error = uveddi::core::interfaces::persistence::PersistenceError>>,
...
impl ExampleAnalysisService {
    ...
    pub async fn analyze_file(&self, file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let completion_event = AnalysisEvent::analysis_completed(
            1, 1, 1, std::time::Duration::from_millis(100)
        );
        self.event_bus.publish(DomainEvent::Analysis(completion_event))?;
