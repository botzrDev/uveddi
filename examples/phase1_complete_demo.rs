...
fn demo_cli_improvements() {
    ...
    println!(
        "   📍 Context: Uveddi supports analysis, configuration, and health checking commands"
    );
    println!("   💡 Suggestions:");
...
fn demo_health_system() {
    ...
    let checks = [
        ...
        (
            "config_directories",
            "⚠️ ",
            "Some configuration directories are missing (auto-fixable)",
        ),
        (
            "tree_sitter",
            "⚠️ ",
        ...
        (
            "ollama_connectivity",
            "❌",
            "Cannot connect to Ollama at http://localhost:11434 (auto-fixable)",
        ),
    ];
