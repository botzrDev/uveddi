...
fn test_database_validation_with_valid_data() {
    ...
    let analysis_run = db.create_analysis_run(std::path::Path::new("./test")).unwrap();
    ...
    assert!(result.is_ok(), "Valid issues should be stored successfully: {:?}", result.err());
}
