...
impl MarkdownReportGenerator {
    ...
    pub async fn generate_markdown_report(
        ...
        markdown.push_str(&self.generate_architecture_overview(analysis_run, issues).await?);
        markdown.push_str("\n\n");
...
| 🟢 Low | {} | {:.1}% |"#,
            ...
            analysis_run.status,
            critical_issues, if total_issues > 0 { (critical_issues as f64 / total_issues as f64) * 100.0 } else { 0.0 },
            high_issues, if total_issues > 0 { (high_issues as f64 / total_issues as f64) * 100.0 } else { 0.0 },
            medium_issues, if total_issues > 0 { (medium_issues as f64 / total_issues as f64) * 100.0 } else { 0.0 },
            low_issues, if total_issues > 0 { (low_issues as f64 / total_issues as f64) * 100.0 } else { 0.0 }
        );
        ...
        if let Some(insights) = ai_insights {
            if !insights.is_empty() {
                ...
                // Add top AI insights
                let top_insights: Vec<_> = insights.iter()
                    .take(3)
                    .enumerate()
                    .collect();
    ...
    async fn generate_architecture_overview(&mut self, analysis_run: &AnalysisRun, issues: &[ArchitecturalIssue]) -> Result<String, MarkdownReportError> {
        ...
        if !components.is_empty() {
            ...
            match self.generate_system_architecture_diagram(&components).await {
                ...
                Err(e) => {
                    overview.push_str(&format!("*Visual diagrams unavailable - {}: showing text summary*\n\n", e));
                }
            }
            overview.push_str("**Architecture Summary:**\n");
            for component in &components {
                overview.push_str(&format!("- {}: {} (Complexity: {:.1})\n", 
                    component.name, 
    ...
    fn generate_ai_analysis_section(&self, ai_insights: &[AiInsight]) -> String {
        ...
        let medium_confidence: Vec<_> = ai_insights.iter().filter(|i| i.confidence >= 0.6 && i.confidence <= 0.8).collect();
    ...
    fn generate_issues_breakdown(&self, issues: &[ArchitecturalIssue], anti_pattern_types: &HashMap<i64, AntiPatternType>) -> String {
        ...
        for issue in issues {
            ...
            by_type.entry(issue_type).or_insert_with(Vec::new).push(issue);
        }
        ...
        for (issue_type, type_issues) in by_type {
            ...
            for issue in type_issues {
                let severity_emoji = if issue.message.contains("critical") || issue.message.contains("Critical") {
                    "🔴"
    ...
    async fn generate_diagrams_section(&mut self, analysis_run: &AnalysisRun, issues: &[ArchitecturalIssue]) -> Result<String, MarkdownReportError> {
        ...
        match self.generate_dependency_diagram(&components).await {
            ...
            Err(e) => {
                diagrams.push_str(&format!("*Visual dependency diagram unavailable - {}*\n\n", e));
            }
        }
        diagrams.push_str("**Component Dependencies:**\n");
        for component in &components {
            if !component.dependencies.is_empty() {
                diagrams.push_str(&format!("- {} depends on {} components\n", 
        ...
        match self.generate_component_interaction_diagram(&components).await {
            Ok(diagram) => {
                diagrams.push_str("```mermaid\n");
                diagrams.push_str(&diagram);
                diagrams.push_str("\n```\n\n");
            ...
            Err(e) => {
                diagrams.push_str(&format!("*Visual interaction diagram unavailable - {}*\n\n", e));
            }
        }
        diagrams.push_str("**Component Summary:**\n");
        for component in &components {
            diagrams.push_str(&format!("- {}: Located at `{}`\n", 
                component.name, 
    ...
    fn generate_recommendations(&self, issues: &[ArchitecturalIssue], ai_insights: Option<&[AiInsight]>) -> String {
        let mut recommendations = String::from("## Recommendations\n\n");
        
        if issues.is_empty() {
            recommendations.push_str("🎉 **Excellent!** No significant architectural issues were found.\n\n");
        ...
        if critical_issues > 0 {
            ...
            for issue in issues.iter().filter(|i| i.message.contains("critical") || i.message.contains("Critical")) {
                recommendations.push_str(&format!(
                    "- **Issue #{}**: {}\n",
                    issue.issue_id.unwrap_or(0),
                    issue.message
        ...
        if high_issues > 0 {
            ...
            for issue in issues.iter().filter(|i| i.message.contains("high") || i.message.contains("High")).take(5) {
                recommendations.push_str(&format!(
                    "- **Issue #{}**: {}\n",
                    issue.issue_id.unwrap_or(0),
                    issue.message
        ...
        if let Some(insights) = ai_insights {
            let actionable_insights: Vec<_> = insights.iter()
        ...
        recommendations.push_str("1. **Address Critical Issues First**: Focus on critical and high-severity issues\n");
        recommendations.push_str("2. **Implement Gradual Refactoring**: Make incremental improvements\n");
        recommendations.push_str("3. **Add Automated Testing**: Ensure changes don't introduce regressions\n");
        recommendations.push_str("4. **Code Review Process**: Implement peer reviews for architectural changes\n");
        recommendations.push_str("5. **Documentation**: Update architectural documentation after changes\n");
    ...
    fn calculate_health_score(&self, issues: &[ArchitecturalIssue]) -> f64 {
        ...
        for issue in issues {
            let deduction = if issue.message.contains("critical") || issue.message.contains("Critical") {
                20.0
            } else if issue.message.contains("high") || issue.message.contains("High") {
                10.0
            } else if issue.message.contains("medium") || issue.message.contains("Medium") {
    ...
    fn extract_architectural_components(&self, analysis_run: &AnalysisRun, issues: &[ArchitecturalIssue]) -> Vec<ArchitecturalComponent> {
            ...
            if seen_files.insert(file_path.clone()) {
                    ...
                    let component = ArchitecturalComponent {
                        ...
                        metrics: ComponentMetrics {
                            complexity: Some(issues.iter()
                                .filter(|i| &i.file_path == file_path)
                                .count() as f64),
    ...
    async fn generate_system_architecture_diagram(&mut self, components: &[ArchitecturalComponent]) -> Result<String, MarkdownReportError> {
        if components.is_empty() {
    ...
    async fn generate_dependency_diagram(&mut self, components: &[ArchitecturalComponent]) -> Result<String, MarkdownReportError> {
        if components.is_empty() {
    ...
    async fn generate_component_interaction_diagram(&mut self, components: &[ArchitecturalComponent]) -> Result<String, MarkdownReportError> {
        if components.is_empty() {
            return Ok("sequenceDiagram\n    Note over System: No Component Interactions Detected".to_string());
        }
        ...
        if diagram == "graph LR\n" {
            diagram = "graph LR\n    A[Independent Components] --> B[No Interactions Detected]".to_string();
        }
    ...
    fn generate_fallback_dependency_diagram(&self, components: &[ArchitecturalComponent]) -> String {
        let mut diagram = String::from("graph LR\n");
