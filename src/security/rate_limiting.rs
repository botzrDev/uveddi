...
use tower_governor::{governor::GovernorConfigBuilder, governor::GovernorConfig};

/// Rate limiter trait for different implementations
#[async_trait::async_trait]
pub trait RateLimitStorage: Send + Sync {
    /// Check if request is within rate limit
    async fn check_rate_limit(&self, key: &str, limit: u32, window_seconds: u32) -> SecurityResult<bool>;
    ...
    async fn check_rate_limit(&self, key: &str, limit: u32, window_seconds: u32) -> SecurityResult<bool> {
        ...
        match data.get_mut(key) {
            ...
            None => {
                ...
                data.insert(key.to_string(), RateLimitEntry {
                    count: 1,
                    window_start: now,
...
impl RateLimitStorage for RedisRateLimitStorage {
    async fn check_rate_limit(&self, _key: &str, _limit: u32, _window_seconds: u32) -> SecurityResult<bool> {
...
impl RateLimiter {
    ...
    pub fn new_with_storage(storage: Arc<dyn RateLimitStorage>, config: RateLimitingConfig) -> Self {
        Self { storage, config }
    }
    ...
    pub async fn check_rate_limit(&self, identifier: &str, endpoint: &str) -> SecurityResult<bool> {
        ...
        // Check rate limit
        self.storage.check_rate_limit(&key, limit, window_seconds).await
    }
    
    /// Get rate limit information for an identifier
    pub async fn get_rate_limit_info(&self, identifier: &str, endpoint: &str) -> SecurityResult<Option<RateLimitInfo>> {
        let key = format!("{}:{}", identifier, endpoint);
    ...
    pub async fn get_remaining_requests(&self, identifier: &str, endpoint: &str) -> SecurityResult<u32> {
...
pub struct RateLimitMiddlewareConfig {
    ...
    pub identifier_extractor: Option<Box<dyn Fn(&axum::http::HeaderMap, &axum::extract::Request) -> String + Send + Sync>>,
    /// Custom endpoint extractor
    pub endpoint_extractor: Option<Box<dyn Fn(&axum::extract::Request) -> String + Send + Sync>>,
    /// Custom rate limit exceeded handler
    pub rate_limit_exceeded_handler: Option<Box<dyn Fn() -> axum::response::Response + Send + Sync>>,
}
...
mod tests {
    ...
    async fn test_rate_limiter_different_identifiers() {
        ...
        let ip_allowed = limiter.check_rate_limit("ip:192.168.1.1", "/api/test").await.unwrap();
        assert!(ip_allowed);
        
        let user_allowed = limiter.check_rate_limit("user:123", "/api/test").await.unwrap();
        assert!(user_allowed);
        
        let api_key_allowed = limiter.check_rate_limit("api_key:abc123", "/api/test").await.unwrap();
    ...
    async fn test_endpoint_specific_limits() {
        ...
        config.endpoint_limits.insert("/api/auth/login".to_string(), 2);
        
        let limiter = RateLimiter::new_in_memory(config);
        
        // First two requests should be allowed
        assert!(limiter.check_rate_limit("ip:192.168.1.1", "/api/auth/login").await.unwrap());
        assert!(limiter.check_rate_limit("ip:192.168.1.1", "/api/auth/login").await.unwrap());
        ...
        assert!(limiter.check_rate_limit("ip:192.168.1.1", "/api/projects").await.unwrap());
    }
    ...
    async fn test_rate_limit_reset() {
        ...
        // Fill up rate limit
        for _ in 0..60 {
            limiter.check_rate_limit("ip:192.168.1.1", "/api/test").await.unwrap();
        }
        
        // Should be rate limited
        assert!(!limiter.check_rate_limit("ip:192.168.1.1", "/api/test").await.unwrap());
        
        // Reset rate limit
        limiter.reset_rate_limit("ip:192.168.1.1", "/api/test").await.unwrap();
    ...
    async fn test_rate_limit_manager() {
        ...
        // Reset rate limit
        manager.reset_rate_limit("test_id", "/api/test").await.unwrap();
