...
pub fn is_analyzable_file(path: &Path) -> bool {
    if let Some(extension) = path.extension().and_then(|ext| ext.to_str()) {
        matches!(extension, "rs" | "py" | "js" | "ts" | "tsx" | "jsx" | "go" | "java" | "c" | "cpp" | "h" | "hpp")
    } else {
        false
    }
}
        ...
        for entry in entries {
            ...
            if path.is_dir() {
                ...
                if !dir_name.starts_with('.') && 
                   !matches!(dir_name, "node_modules" | "target" | "dist" | "build" | "__pycache__") {
...
fn parse_analysis_output(output: &[u8]) -> (u32, u32, String, Vec<String>) {
    if let Ok(json_str) = String::from_utf8_lossy(output).parse::<serde_json::Value>() {
        let issues = json_str.get("issues").and_then(|i| i.as_array()).map(|arr| arr.len()).unwrap_or(0) as u32;
        let critical = json_str.get("critical_issues").and_then(|i| i.as_u64()).unwrap_or(0) as u32;
        let message = json_str.get("summary").and_then(|s| s.as_str()).unwrap_or("Analysis completed").to_string();
        let suggestions = json_str.get("suggestions")
...
mod tests {
    ...
    fn test_should_skip_analysis() {
        ...
        assert!(should_skip_analysis("WIP: working on feature", &skip_patterns));
        assert!(should_skip_analysis("Fix bug [skip ci]", &skip_patterns));
        assert!(!should_skip_analysis("Regular commit message", &skip_patterns));
    }
