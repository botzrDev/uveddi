...
impl RestApiService {
    ...
    pub fn create_app_with_state(&self) -> Router {
        ...
        let api_routes = Router::new()
            ...
            .route("/cache/control", post(control_cache))
            .route("/cache/warmup", post(warmup_cache))
            .route("/cache/efficiency", get(get_cache_efficiency))
            .route("/cache/health", get(get_cache_health))
            // Knowledge Graph endpoints (P1 - Graph operations)
            .route("/graph/query", post(execute_graph_query))
            .route("/graph/visualization", post(get_graph_visualization))
            .route("/graph/analytics", get(get_graph_analytics))
            .route("/graph/update/{file_path}", post(update_graph_incremental))
            // Streaming endpoints (WebSocket)
            .route("/stream/info", get(get_streaming_info))
            .route("/stream/analysis/{id}", get(stream_analysis_progress_ws))
            .route("/stream/cache", get(stream_cache_stats_ws))
            .route("/stream/events", get(stream_events_ws))
            // Report endpoints
