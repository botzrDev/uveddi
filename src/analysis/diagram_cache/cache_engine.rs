...
use super::{
    DiagramType, CachedDiagram, DiagramCacheStats, DiagramCacheConfig, 
    DiagramDependencyTracker, InvalidationManager, CompressionEngine,
    Result, DiagramCacheError, CacheWarmingStrategy
};
...
impl DiagramCacheEngine {
    ...
    pub fn new(config: DiagramCacheConfig) -> Self {
        ...
        for diagram_type in [DiagramType::Mermaid, DiagramType::PlantUML, DiagramType::Graphviz] {
            let cache_size = std::num::NonZero::new(config.max_cached_diagrams / 3).unwrap_or(std::num::NonZero::new(1000).unwrap());
            cache_layers.insert(
                diagram_type,
    ...
    pub async fn get_diagram(&self, diagram_id: &str, diagram_type: &DiagramType) -> Result<Option<Vec<u8>>> {
        ...
        if let Some(cache_layer) = self.cache_layers.get(diagram_type) {
            ...
            if let Some(cached_diagram) = cache.get_mut(diagram_id) {
                ...
                // Decompress content
                let content = self.compression_engine.decompress(&cached_diagram.content, diagram_type).await?;
                ...
                debug!("Cache hit for diagram: {} (type: {:?})", diagram_id, diagram_type);
        ...
        debug!("Cache miss for diagram: {} (type: {:?})", diagram_id, diagram_type);
        Ok(None)
    }
    ...
    pub async fn store_diagram(
        ...
        // Compress content
        let compressed_content = self.compression_engine.compress(&content, &diagram_type).await?;
        ...
        self.dependency_tracker.add_diagram_dependencies(
            &diagram_id,
            &diagram_type,
            &dependencies,
        ).await?;
        ...
        info!("Stored diagram in cache: {} (type: {:?}, compression: {:.1}%)", 
    ...
    pub async fn invalidate_from_changeset(&self, changeset: &ChangeSet) -> Result<()> {
        ...
        let affected_diagrams = self.dependency_tracker
        ...
        for (diagram_type, cache_layer) in &self.cache_layers {
            ...
            for diagram_id in to_remove {
                if cache.pop(&diagram_id).is_some() {
                    total_invalidated += 1;
                    debug!("Invalidated diagram: {} (type: {:?})", diagram_id, diagram_type);
                }
            }
        }
        
        // Update dependency tracker
        self.dependency_tracker.remove_affected_dependencies(&affected_diagrams).await?;
        
        // Update statistics
        let mut stats = self.stats.write().await;
        stats.invalidations += total_invalidated;
        stats.cached_diagrams = stats.cached_diagrams.saturating_sub(total_invalidated as usize);
        
        let invalidation_time = start_time.elapsed().as_millis();
        info!("Invalidated {} diagrams in {}ms based on changeset", 
    ...
    pub async fn selective_regeneration(&self, changeset: &ChangeSet) -> Result<Vec<String>> {
        ...
        let diagrams_to_regenerate = self.dependency_tracker
        ...
        info!("Identified {} diagrams for selective regeneration", diagrams_to_regenerate.len());
    ...
    pub async fn warm_cache(&self, root_path: &Path) -> Result<()> {
        match self.config.warming_strategy {
            ...
            CacheWarmingStrategy::DependencyBased => self.warm_cache_dependency_based(root_path).await,
