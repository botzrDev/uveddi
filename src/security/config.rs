...
impl SecurityConfigLoader {
    ...
    pub fn build(self) -> SecurityResult<SecurityConfig> {
        let config = self.config_builder.build().map_err(|e| SecurityError::ConfigurationError {
            message: format!("Failed to build configuration: {}", e),
        })?;
        
        let security_config = config.try_deserialize::<SecurityConfig>().map_err(|e| SecurityError::ConfigurationError {
...
impl SecurityConfig {
    ...
    fn validate_authentication(&self) -> SecurityResult<()> {
        ...
        for provider in &self.authentication.oauth_providers {
            if provider.client_id.is_empty() {
                return Err(SecurityError::ConfigurationError {
                    message: format!("OAuth provider '{}' client ID cannot be empty", provider.provider_name),
                });
            }
            
            if provider.client_secret.is_empty() {
                return Err(SecurityError::ConfigurationError {
                    message: format!("OAuth provider '{}' client secret cannot be empty", provider.provider_name),
                });
            }
            
            if provider.auth_url.is_empty() {
                return Err(SecurityError::ConfigurationError {
                    message: format!("OAuth provider '{}' auth URL cannot be empty", provider.provider_name),
                });
            }
            
            if provider.token_url.is_empty() {
                return Err(SecurityError::ConfigurationError {
                    message: format!("OAuth provider '{}' token URL cannot be empty", provider.provider_name),
                });
            }
        }
        ...
        for provider in &self.authentication.oidc_providers {
            if provider.client_id.is_empty() {
                return Err(SecurityError::ConfigurationError {
                    message: format!("OIDC provider '{}' client ID cannot be empty", provider.provider_name),
                });
            }
            
            if provider.client_secret.is_empty() {
                return Err(SecurityError::ConfigurationError {
                    message: format!("OIDC provider '{}' client secret cannot be empty", provider.provider_name),
                });
            }
            
            if provider.issuer_url.is_empty() {
                return Err(SecurityError::ConfigurationError {
                    message: format!("OIDC provider '{}' issuer URL cannot be empty", provider.provider_name),
                });
            }
        }
            ...
            match self.audit.store_type.as_str() {
                ...
                "database" => {
                    if self.audit.database_url.is_none() {
                        return Err(SecurityError::ConfigurationError {
                            message: "Database URL is required for database audit store".to_string(),
                        });
                    }
                }
                "file" => {
    ...
    fn validate_api_security(&self) -> SecurityResult<()> {
        ...
        if self.api_security.enable_request_signing {
            ...
            if !valid_algorithms.contains(&self.api_security.signing_algorithm.as_str()) {
                return Err(SecurityError::ConfigurationError {
                    message: format!("Invalid signing algorithm: {}", self.api_security.signing_algorithm),
                });
            }
        }
    ...
    fn validate_secret_management(&self) -> SecurityResult<()> {
        ...
        if !valid_stores.contains(&self.secrets.primary_store.as_str()) {
            return Err(SecurityError::ConfigurationError {
                message: format!("Invalid primary secret store: {}", self.secrets.primary_store),
            });
        }
...
mod tests {
    ...
    fn test_config_file_operations() {
        ...
        assert_eq!(loaded_config.authentication.jwt_expiry_hours, config.authentication.jwt_expiry_hours);
    ...
    fn test_config_builder() {
        ...
        loader = loader.add_file(default_config_path.to_str().unwrap(), false).unwrap();
    ...
    fn test_oauth_provider_validation() {
        ...
        config.authentication.oauth_providers.push(OAuthProviderConfig {
    ...
    fn test_oidc_provider_validation() {
        ...
        config.authentication.oidc_providers.push(OidcProviderConfig {
            provider_name: "test".to_string(),
        ...
        // Fix OIDC provider
        config.authentication.oidc_providers[0].issuer_url = "https://example.com/.well-known/openid_configuration".to_string();
        assert!(config.validate().is_ok());
    }
