...
use uveddi::analysis::diagram_cache::{
    DiagramCacheEngine, DiagramCacheConfig, DiagramType, CachedDiagram
};
            ...
            |b, &count| {
                b.to_async(&rt).iter(|| async {
                    ...
                    for i in 0..count {
                        ...
                        let _ = cache_engine.store_diagram(
                            diagram_id,
                            DiagramType::Mermaid,
                            content.into_bytes(),
                            dependencies,
                ...
                b.to_async(&rt).iter_setup(
                    ...
                    |cache_engine| async move {
                        ...
                        for i in 0..100 {
                            ...
                            let _ = cache_engine.get_diagram(&diagram_id, &DiagramType::Mermaid).await;
                        }
    ...
    c.bench_function("cache_hit_rate_90_percent", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            for i in 0..1000 {
                ...
                if let Ok(Some(_)) = cache_engine.get_diagram(&diagram_id, &DiagramType::Mermaid).await {
                    hits += 1;
                } else {
                    misses += 1;
                }
            ...
            assert!(hit_rate >= 0.9, "Cache hit rate should be >= 90%, got {:.1}%", hit_rate * 100.0);
                ...
                b.to_async(&rt).iter_setup(
                    || {
                        ...
                        rt.block_on(async {
                            ...
                            for i in 0..1000 {
                                let diagram_id = format!("diagram_{}", i);
                                let content = format!("graph TD\n    A{} --> B{}\n    B{} --> C{}", i, i, i, i);
                                let dependencies = vec![PathBuf::from(format!("file_{}.rs", i % 100))];
                    ...
                    |cache_engine| async move {
                        ...
                        let changeset = ChangeSet {
                            ...
                            detection_method: uveddi::analysis::incremental::ChangeDetectionMethod::FileSystem,
                        };
        ...
        b.to_async(&rt).iter_setup(
            ...
            |cache_engine| async move {
                ...
                assert!(avg_lookup_time < 50.0, 
                       "Average lookup time should be <50ms, got {:.1}ms", avg_lookup_time);
