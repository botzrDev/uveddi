    ...
    pub fn is_rate_limit_error(&self) -> bool {
        matches!(
            ...
            SecurityError::RateLimitExceeded { .. }
                | SecurityError::RateLimitingUnavailable
        )
    }
...
impl From<oauth2::RequestTokenError<oauth2::reqwest::Error<reqwest::Error>, oauth2::StandardErrorResponse<oauth2::basic::BasicErrorResponseType>>> for SecurityError {
    fn from(error: oauth2::RequestTokenError<oauth2::reqwest::Error<reqwest::Error>, oauth2::StandardErrorResponse<oauth2::basic::BasicErrorResponseType>>) -> Self {
        SecurityError::OAuth2Error {
            error: error.to_string(),
        }
    }
...
impl From<reqwest::Error> for SecurityError {
    fn from(error: reqwest::Error) -> Self {
        SecurityError::NetworkError {
            endpoint: error.url().map(|u| u.to_string()).unwrap_or_else(|| "unknown".to_string()),
            error: error.to_string(),
        }
    }
}
