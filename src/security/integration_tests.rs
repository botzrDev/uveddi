...
mod tests {
    use crate::security::{
        ...
        models::{AuditEventType, AuditOutcome, AuthContext, User, UserRole},
    ...
    async fn test_complete_authentication_flow() {
        ...
        let (api_key, _) = auth_service
            .generate_api_key(Some(auth_user.id), "Test API Key".to_string(), None)
            .await
            .unwrap();
    ...
    async fn test_complete_authorization_flow() {
        ...
        let allowed = engine
            .check_permission(&user_id, "projects", "read", &context)
            .await
            .unwrap();
    ...
    async fn test_rate_limiting_integration() {
        ...
        for _ in 0..60 {
            let allowed = limiter
                .check_rate_limit(ip_identifier, "/api/test")
                .await
                .unwrap();
            assert!(allowed);
        }
        ...
        let allowed = limiter
            .check_rate_limit(user_identifier, "/api/test")
            .await
            .unwrap();
        assert!(allowed);
        ...
        let allowed = limiter
            .check_rate_limit(api_key_identifier, "/api/test")
            .await
            .unwrap();
        assert!(allowed);
        ...
        let allowed = limiter
            .check_rate_limit(ip_identifier, "/api/auth/login")
            .await
            .unwrap();
    ...
    async fn test_security_error_handling() {
        ...
        assert_eq!(
            auth_error.severity(),
            crate::security::errors::SecurityErrorSeverity::High
        );
        assert_eq!(
            rate_limit_error.severity(),
            crate::security::errors::SecurityErrorSeverity::Medium
        );
    }
    ...
    async fn test_permission_inheritance() {
        ...
        let admin_allowed = engine
            .check_role_permission(&UserRole::Admin, "projects", "delete", &admin_context)
            .await
            .unwrap();
        ...
        let dev_allowed = engine
            .check_role_permission(&UserRole::Developer, "projects", "read", &dev_context)
            .await
            .unwrap();
        assert!(dev_allowed);

        // Test developer cannot delete
        let dev_delete_allowed = engine
            .check_role_permission(&UserRole::Developer, "projects", "delete", &dev_context)
            .await
            .unwrap();
    ...
    async fn test_concurrent_access() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                ...
                let allowed = limiter
                    .check_rate_limit(&identifier, "/api/test")
                    .await
                    .unwrap();
                allowed
            });
