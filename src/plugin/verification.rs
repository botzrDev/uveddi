...
impl PluginVerifier {
    pub fn verify_plugin(
        wasm_path: &Path,
        manifest_path: &Path,
    ) -> Result<PluginManifest, UveddiError> {
    ...
    fn verify_file_hash(wasm_path: &Path) -> Result<(), UveddiError> {
        ...
        if contents.len() < 100 {
            return Err(UveddiError::PluginError(
                "WASM file too small to be valid".to_string(),
            ));
        }
        ...
        if contents.len() > 50 * 1024 * 1024 {
            // 50MB limit
            return Err(UveddiError::PluginError(
                "WASM file too large (>50MB)".to_string(),
            ));
        }
    ...
    fn validate_manifest(manifest: &PluginManifest) -> Result<(), UveddiError> {
        ...
        if manifest.name.len() > 100 {
            return Err(UveddiError::PluginError(
                "Plugin name too long (>100 chars)".to_string(),
            ));
        }
        ...
        if !Self::is_valid_version(&manifest.version) {
            return Err(UveddiError::PluginError(
                "Invalid version format (use semver)".to_string(),
            ));
        }
        ...
        if manifest.permissions.len() > 20 {
            return Err(UveddiError::PluginError(
                "Too many permissions requested (max 20)".to_string(),
            ));
        }
