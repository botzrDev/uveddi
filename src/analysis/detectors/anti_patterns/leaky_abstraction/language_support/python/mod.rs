...
impl PythonLanguageSupport {
    ...
    pub fn analyze_file(
        ...
        issues.extend(self.class_analyzer.analyze_class_internals(parsed_file, context)?);
        issues.extend(self.class_analyzer.analyze_private_methods(parsed_file, context)?);

        // Analyze import violations
        issues.extend(self.import_analyzer.analyze_import_violations(parsed_file, context)?);

        // Analyze framework coupling
        issues.extend(self.framework_analyzer.analyze_framework_coupling(parsed_file, context)?);
    ...
    pub fn extract_python_import_module(&self, import_text: &str) -> Option<String> {
        self.import_analyzer.extract_python_import_module(import_text)
    }
}
