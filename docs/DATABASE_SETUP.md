# Uveddi Database Setup Guide

This guide provides comprehensive setup instructions for both the local SQLite database (Rust CLI) and the centralized PostgreSQL database (Python/FastAPI backend) for the Uveddi project.

## Part 1: Local SQLite Database Setup (Rust CLI)

### Overview
The Uveddi CLI uses SQLite for local storage of analysis results, cached ASTs, and user-specific configurations. This allows for privacy-focused, offline analysis and is crucial for the free tier.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Initialize database (creates .uveddi/data.db by default)
...
- Default location: `.uveddi/data.db` in the project root
...
Example:
```rust
use uveddi::database::{DatabaseManager, ArchitecturalIssue, CodeSnippet};
...
### Overview
The centralized database serves the Uveddi subscriber system, CI/CD integrations, and manages centralized analysis data using PostgreSQL for scalability and robustness.
...
- **User**: Uveddi subscribers with role-based access
...
#### Local (SQLite)
- Database file in `.uveddi/` directory (gitignored)
...
This setup provides a robust, scalable foundation for Uveddi while maintaining the ability to operate offline with local SQLite storage and scale to enterprise needs with the centralized PostgreSQL backend.
