...
use super::{
    ...
    memory_tracker::{MemoryTracker, MemoryGuard},
    resource_config::{ResourceConfig, ProcessingLimits},
};
...
impl AnalysisOrchestrator {
    ...
    pub fn new(
        ...
        let concurrency_limiter = Arc::new(Semaphore::new(
            processing_limits.max_concurrent_analyses
        ));
    ...
    pub async fn start_analysis<'a>(
        ...
        if self.can_start_immediately(estimated_memory).await {
            self.execute_analysis(analysis_id, component, estimated_memory).await
        } else {
            self.queue_analysis(analysis_id, component, estimated_memory, priority).await
        }
    }
    ...
    pub async fn update_limits(&self, config: &ResourceConfig) -> ResourceResult<()> {
        ...
        *current_config = config.processing.clone();
        ...
        if target_permits > current_permits {
            // Add permits
            self.concurrency_limiter.add_permits(target_permits - current_permits);
        }
    ...
    pub async fn emergency_cleanup(&self) -> ResourceResult<()> {
        ...
        let critical_analyses: Vec<_> = queue.drain(..)
    ...
    pub async fn process_queue(&self) -> ResourceResult<usize> {
        ...
        loop {
            ...
            if self.can_start_immediately(next_analysis.estimated_memory).await {
                match self.execute_analysis(
                    next_analysis.id,
    ...
    async fn execute_analysis<'a>(
        ...
        // Allocate memory
        let memory_guard = self.memory_tracker
            .allocate(component, estimated_memory)?;
        ...
        self.active_analyses.lock().unwrap().insert(analysis_id, analysis_info);
    ...
    async fn queue_analysis<'a>(
        ...
        loop {
            ...
            if self.can_start_immediately(estimated_memory).await {
                return self.execute_analysis(analysis_id, component, estimated_memory).await;
            }
        }
    }
}
...
impl<'a> Drop for AnalysisExecution<'a> {
    fn drop(&mut self) {
        ...
        if self.permit.is_some() || self.memory_guard.is_some() {
            self.cleanup(false, Some(ResourceError::SystemError(
