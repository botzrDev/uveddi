...
fn test_memory_pool_basic_functionality() {
    ...
    let pool = MemoryPool::<Vec<String>>::new(
        10,
...
fn test_pool_pre_population() {
    let pool = MemoryPool::<Vec<i32>>::new(
        20,
...
fn test_pool_statistics_and_efficiency() {
    let pool = MemoryPool::<String>::new(
        100,
        AllocationStrategy::AdaptiveBased { target_memory: 1024 },
        "stats_test"
    );
...
fn test_concurrent_pool_access() {
    let pool = Arc::new(MemoryPool::<Vec<u64>>::new(
        50,
        AllocationStrategy::GrowthBased { initial: 10, growth_factor: 1.5 },
        "concurrent_test"
    ));
...
fn test_memory_optimization_with_pools() {
    ...
    assert!(result.is_ok(), "Memory optimization with pools should initialize successfully");
...
fn test_allocation_strategies() {
    ...
    let fixed_pool = MemoryPool::<i32>::new(
        10,
        AllocationStrategy::FixedSize(10),
        "fixed_test"
    );
    ...
    let growth_pool = MemoryPool::<i32>::new(
        10,
        AllocationStrategy::GrowthBased { initial: 5, growth_factor: 2.0 },
        "growth_test"
    );
