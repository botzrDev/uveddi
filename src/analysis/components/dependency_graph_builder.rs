...
use super::traits::{DependencyGraphBuilder, AstProvider};
...
impl DependencyGraphBuilderImpl {
    ...
    async fn extract_file_dependencies(&self, file_path: &Path) -> Result<Vec<Dependency>, UveddiError> {
        ...
        self.dependency_extractor.extract_from_ast(&parsed_file)
...
impl DependencyGraphBuilder for DependencyGraphBuilderImpl {
    async fn build_graph(&self, root_path: &Path) -> Result<LocalDependencyGraph, UveddiError> {
        info!("Building dependency graph from root path: {}", root_path.display());
                ...
                Ok(file_path) => {
                    match self.extract_file_dependencies(&file_path).await {
                        Ok(mut file_dependencies) => {
                            info!("Extracted {} dependencies from {}", 
                                  file_dependencies.len(), file_path.display());
                            all_dependencies.append(&mut file_dependencies);
                            files_processed += 1;
                        }
                        Err(e) => {
                            warn!("Failed to extract dependencies from {}: {}", 
                                  file_path.display(), e);
                        }
                    }
                }
        ...
        info!("Processed {} files, found {} total dependencies", 
              files_processed, all_dependencies.len());
