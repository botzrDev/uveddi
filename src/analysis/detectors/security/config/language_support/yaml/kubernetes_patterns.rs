...
impl KubernetesPatternChecker {
    ...
    pub fn check_k8s_security_context(security_context: &YamlValue, issues: &mut Vec<ConfigIssue>) {
        if let YamlValue::Mapping(map) = security_context {
            ...
            if let Some(allow_priv_esc) =
                map.get(&YamlValue::String("allowPrivilegeEscalation".to_string()))
            {
            ...
            if let Some(read_only_fs) =
                map.get(&YamlValue::String("readOnlyRootFilesystem".to_string()))
            {
            ...
            for volume in volume_list {
                if let YamlValue::Mapping(volume_map) = volume {
                    ...
                    if let Some(host_path) =
                        volume_map.get(&YamlValue::String("hostPath".to_string()))
                    {
                        if let YamlValue::Mapping(host_path_map) = host_path {
                            if let Some(path) =
                                host_path_map.get(&YamlValue::String("path".to_string()))
                            {
                                if let Some(path_str) = path.as_str() {
                                    ...
                                    if sensitive_paths
                                        .iter()
                                        .any(|&sensitive| path_str.starts_with(sensitive))
                                    {
                    ...
                    if let Some(mount_path) =
                        volume_map.get(&YamlValue::String("mountPath".to_string()))
                    {
                        if let Some(mount_path_str) = mount_path.as_str() {
                            let read_only = volume_map
                                .get(&YamlValue::String("readOnly".to_string()))
                                .and_then(|v| v.as_bool())
                                .unwrap_or(false);

                            if !read_only
                                && (mount_path_str.starts_with("/etc")
                                    || mount_path_str.starts_with("/proc"))
                            {
                                issues.push(
                                    utils::create_config_issue(
                                        ConfigSeverity::Medium,
                                        ...
                                        format!(
                                            "Writable mount of sensitive path: {}",
                                            mount_path_str
                                        ),
                                        None,
        ...
        if let YamlValue::Mapping(cap_map) = capabilities {
            ...
            if let Some(drop_caps) = cap_map.get(&YamlValue::String("drop".to_string())) {
                if let YamlValue::Sequence(drop_list) = drop_caps {
                    let has_all = drop_list.iter().any(|cap| cap.as_str() == Some("ALL"));
    ...
    pub fn check_resource_limits(map: &serde_yaml::Mapping, issues: &mut Vec<ConfigIssue>) {
        ...
        if let Some(resources) = map.get(&YamlValue::String("resources".to_string())) {
            if let YamlValue::Mapping(resources_map) = resources {
                ...
                let has_limits =
                    resources_map.contains_key(&YamlValue::String("limits".to_string()));
                let has_requests =
                    resources_map.contains_key(&YamlValue::String("requests".to_string()));
...
mod tests {
    ...
    fn test_root_user_detection() {
        ...
        let security_context = serde_yaml::from_str(
            r#"
runAsUser: 0
...
"#,
        ...
        assert!(issues
            .iter()
            .any(|i| i.title.contains("Privilege Escalation")));
    }

    #[test]
    fn test_sensitive_host_path_detection() {
        let mut issues = Vec::new();
        let volumes = serde_yaml::from_str(
            r#"
- name: docker-sock
  hostPath:
...
"#,
        ...
        assert!(issues
            .iter()
            .any(|i| i.title.contains("Sensitive Host Path")));
    }

    #[test]
    fn test_dangerous_capability_detection() {
        let mut issues = Vec::new();
        let capabilities = serde_yaml::from_str(
            r#"
add:
- SYS_ADMIN
- NET_ADMIN
"#,
        ...
        assert!(issues
            .iter()
            .any(|i| i.title.contains("Dangerous Capability")));
    }
}
